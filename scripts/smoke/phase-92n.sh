...
skip "phase 92n: resume-completes-attach bridge — replace with real assertions when the phase lands (drive an admin add_mcp_connection that parks auth_required, resume the pause token, assert the connection reaches online + a terminal mcp.connection.added; a forced re-drive failure surfaces mcp.connection.failed with the server offline)"
