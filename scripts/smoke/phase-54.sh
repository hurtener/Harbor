...
# conformance and reusable client trees built on top of them.
if grep -rIn --include='*.go' '"net/http"' "${PROTO_PKG}/" 2>/dev/null | grep -v '/transports/' | grep -v '/auth/' | grep -v '/conformance/' | grep -v '/client/' | grep -q .; then
