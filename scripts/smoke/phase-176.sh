...
skip "phase 176: smoke skeleton — replace with real assertions when session reopen lands (start on a closed session id resumes with history; start on an erased id → reopen-after-erase; cross-tenant start on a closed id rejected)"
