...
# D-093: protocol.ts must carry the generated-file header.
if [ -f "web/console/src/lib/protocol.ts" ]; then
    if grep -q 'CODE GENERATED BY cmd/harbor-gen-protocol-ts' "web/console/src/lib/protocol.ts"; then
        ok "phase-72h: protocol.ts carries the generated-file header (D-093)"
    else
        fail "phase-72h: protocol.ts missing the generated-file header (D-093)"
    fi
fi
