...
assert_grep_present 'TestExternalGrantReadiness_StockOutboxReplayFailureDegradesThenRecovers' "internal/runtime/serve/external_grant_transport_test.go" "stock replay failure projects degraded runtime readiness"
assert_grep_present 'TestOutboxRunReplayStoreFailureDegradesThenRecovers' "internal/llm/receipts/outbox_due_test.go" "transient durable replay failure recovers through bounded worker retry"
assert_grep_present 'TestOutboxRunCorruptDueIndexDegradesWithoutHotLoopAndClosesQuietly' "internal/llm/receipts/outbox_due_test.go" "corrupt due index remains degraded without a worker hot loop"
