...
5. **Typed Protocol client, lockstep with `CanonicalWireTypes` (D-093).** The Console talks to the Runtime through a typed client at `web/console/src/lib/protocol.ts`, kept in lockstep with `internal/protocol/singlesource.CanonicalWireTypes`. D-093 specifies a `cmd/harbor-gen-protocol-ts` generator (with a `make protocol-ts-gen-check` CI gate) that would regenerate this file from the Go single source — **that generator has NOT been built yet** (Phase 72h committed `protocol.ts` as a hand-shaped stub; later phases hand-extended it; the Wave 13 §17.5 checkpoint (D-132/W10) corrected the formerly-false `// CODE GENERATED … DO NOT EDIT` header to the accurate hand-maintained notice the file now carries, and filed a tracking issue for building the generator + gate). **Until the generator lands, `protocol.ts` is hand-maintained: any Go-side wire-type change MUST be mirrored into it by hand**, and hand-rolled `fetch` calls in components are still not allowed (go through the typed client). When the generator + `make protocol-ts-gen-check` gate ship, this rule reverts to "generated, never hand-edited."
