...
var eventPayloadIndex = map[events.EventType]payloadEntry{
	...
	tasks.EventTypeTaskProgress:               {Payloads: []reflect.Type{reflect.TypeOf(tasks.TaskProgressPayload{})}},
	...
	toolauth.EventTypeToolCredentialExchanged: {
		Payloads: []reflect.Type{reflect.TypeOf(toolauth.ToolCredentialExchangedPayload{})},
		Note:     "The `AudienceVerified` field is true only when the general tokenexchange path read a JWT `aud` claim and it included the declared RFC 8707 resource. Signed-capability exchanges prove the destination from the broker response and leave this field false.",
	},
