...
**Related residue from the same revert (not part of this ask, but the same root cause):** `ui/resource-teardown` on unmount, the `request-teardown` handler, host-context `toolInfo` / `containerDimensions`, and `onlistresourcetemplates` are likewise absent from `app-bridge-host.ts` while D-227 item 4 records all of them as shipped. Any phase that picks up HA-38 should decide explicitly whether it also closes these, and the 109k plan / D-227 / the master-plan detail block should be corrected to say what actually shipped.
