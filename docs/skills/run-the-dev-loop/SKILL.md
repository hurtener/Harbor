...
- **`400 invalid_request` from `/v1/dev/bootstrap.json` naming a field.** The bootstrap body accepts exactly four members — `tenant`, `user`, `session`, `scopes` — and refuses anything else by name instead of dropping it, because the endpoint mints a credential. The two common mistakes are `scope` for `scopes` (which used to answer `200` with a full **admin** token when you asked for none) and `tenant_id` / `user_id` / `session_id` (which used to answer `200` with a token for the **default** identity, not the one you named). Use the unsuffixed spellings; `-d '{}'` remains the plain default-token call.
