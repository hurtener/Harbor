...
- **MCP App DisplayMode — `inline` / `fullscreen` / `pip` (Phase 109c).** An MCP App can declare (or request at runtime) a larger layout than the inline chat widget, and the Playground honours it without reloading your session:
  ...
  - **A rendered app survives leaving and reopening the session (Phase 204).** Reopen the conversation and the widget comes back on the same turn it was discovered on, in the same display mode, with the same data — the Console replays the `mcp.app_available` event from the durable stream and re-reads the tool call's captured input/result behind it. You don't re-run anything. If that captured data is no longer available (an unknown or expired record, or a runtime that restarted with the in-memory state driver), the turn shows a plain **"This view is no longer available"** note instead of the widget — never a blank bubble and never an app frame that loads but stays empty. The rest of the turn (its text, tool cards, and stats) is unaffected.
    - **Fresh render admission on durable reopen (HA-56 fresh render-admission amendment, Shipped v1.28).** There are exactly two ways a widget mounts. While the originating tool result is live, the Console may use a **bounded, short-lived, provider-local binding** — a compatibility convenience only; it is never durable and never restored. When the app is remounted from a **reopened session's durable turns**, the Console instead requests a **fresh render admission** — a stateless, integrity-protected claim sealed by the runtime's existing deployment-shared KEK-backed sealer (`tools.oauth_token_kek_env`) — and the runtime mints it only after re-checking your verified identity, the signed effective-agent reach, agent retirement, session/agent erasure, the current session/agent tool exposure, the exact server, the exact current `ui://` resource, paused/disabled state, and the exact current provider/catalog generation — the generation of the server's current discovered catalog, which changes on detach, replacement, or any successful discovery/catalog/resource change even when the deployment descriptor configuration did not change. The reopen order is: the durable App reference from the reopened turn, a successful tool-context replay first (a failed / unavailable / evicted / foreign replay mints nothing), the current `ui://` read explicitly requesting one admission, then the iframe mount, then app→tool calls through the same-server wrapped path echoing the admission. The generation is deterministic and replica-stable (a process-local discovery counter is never the generation), so restart and multi-replica reopen succeed only when the shared KEK AND the same current catalog generation are both present, and a replica holding a different current catalog fails closed as a generation mismatch. The claim carries no raw arguments, secrets, provider output, callback names, or general capabilities, and ordinary resource reads never mint one — only the explicit admission-requesting read does. The surface is strictly opt-in — an OAuth broker supplying the shared sealer alone never enables it — and every check reads the reach-admitted effective agent from the request, never a fixed boot/default fallback. The durable turn rows the reopened session pages through carry metadata and component-availability only, never a token; the tool-context replay behind the widget is unchanged and never reruns the originating tool. If the admission is refused or expired you get a typed unavailable/expired result and the honest **"This app's interactive session could not be authorized"** note — distinct from the tool-context-missing **"This view is no longer available"** note; nothing is silently extended, and refreshing re-runs the full fresh checks.
    - **Render admission is opt-in — off by default (HA-56 / D-412).** The
      fresh render-admission surface above enables only when the runtime
      boots with `tools.mcp_app_render_admission.enabled: true` (default
      false, restart-required; an enabled surface also needs a resolvable
      `tools.oauth_token_kek_env` shared KEK, and readiness fails loud
      without one). On an unwired / default-off runtime the Console keeps
      rendering the legacy surface: a currently live tool-result App whose
      ref still carries a valid legacy binding retries the ordinary read
      exactly once and may render through that binding (callbacks on the
      binding alone — never both authorities), while a durable reopen with
      no live binding enters the explicit admission-safe unavailable state —
      no iframe mounts and no app callback is invoked, and the turn shows
      the honest **"This app's interactive session could not be authorized"** note.
  - **The app adapts to your Console theme, and renders with real data — no extra wiring.** When the widget mounts, the host hands the app your light/dark preference (from the OS `prefers-color-scheme`) plus the Console's structural design tokens, so a spec-conformant app themes itself to match instead of booting with a fixed palette; flip your OS light/dark mid-session and the rendered app re-themes live (it is never reloaded — the handshake stays intact). The host also **delivers the originating tool call's input and result into the app after it initializes**, so the app renders its real content (the numbers/rows the tool returned) rather than an empty shell — it reads that pushed data and never re-invokes the tool. Both are automatic: a real Dockyard App (e.g. `analytics-widgets`) renders themed, with its data, on first mount.
    - **Large tool results reach the app too, on whichever artifact store you configured.** A tool result above the heavy-output threshold is stored by reference rather than inlined (that is what keeps it out of the model's context), and the Console reads those bytes back through the artifact read that every driver serves — in-memory, filesystem, SQLite, Postgres, S3. Previously that read went through presigned URLs, which only an S3-compatible store can mint, so on a default deployment a large result reached the app as an `[artifact … ]` placeholder instead of its data. The app now receives the same structured payload whether the result was small enough to inline or not: **size no longer changes the shape an app is given.** If the bytes genuinely cannot be read, the app still gets a placeholder naming the artifact — never silently empty data.
  ...
  - **An app can only call its own server's tools (Phase 207).** When a rendered app invokes a tool, the Console resolves that call inside the app's OWN MCP server's namespace. An app cannot reach a tool on a different server, or any other tool in your catalog, by naming it — and it never picks the server itself; the host derives that from where the app came from. Everything else is unchanged: the call still runs under your identity triple and still hits the same approval / OAuth / paused-server gates. If the app names a tool that does not exist on its server, it gets a clear "not available here" answer it can render around, instead of a generic failure. App-only callback tools stay out of the planner's view, generic tool lists, and search entirely (HA-56 / D-412) — they are never offered to the model and are only reachable through this same-server resolution path, followed by the existing approval / OAuth / policy / redaction / retry / audit gates.
    - **App authors: send the BARE server-side tool name.** Send `get_forecast`, not Harbor's catalog key `weather_get_forecast` — the host adds the server prefix for you. This CHANGED in Phase 207: the host used to pass whatever the app sent straight through, so an app written against the catalog key worked by accident. It now resolves to `weather_weather_get_forecast` and comes back as "not available here". If a previously-working app suddenly reports a missing tool, this is why; strip the server prefix from the names it sends.
    - **Operators: two server names where one is a prefix of the other are refused at startup.** Harbor will not attach both `github` and `github_enterprise` (or any pair where one name plus `_` begins the other) — it fails loudly, naming both, and asks you to rename one. That pairing would make the `<server>_<tool>` naming ambiguous and let an app on the shorter name reach the longer one's tools. Names that merely share a prefix without a `_` boundary (`github` and `githubby`) are fine.
