...
- **MCP App widgets** — when a tool result declares an interactive UI (an MCP server's `ui://` resource via `_meta.ui.resourceUri`), the Console renders it **inline** as a sandboxed-iframe widget in the chat scroll (Phase 109b). The app runs under a strict sandbox (no parent-DOM / cookie / `localStorage` access) and a strict CSP; it cannot open its own network connection. Any tool call the app makes is proxied back through the Harbor Runtime under your identity triple and hits the SAME approval / OAuth gates a planner-initiated call does (D-173) — an app call to a gated tool parks on the same pause card above. Fullscreen / pip layouts land in 109c; 109b is inline-only.
