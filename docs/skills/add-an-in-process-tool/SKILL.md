...
- **A mapped parameter is required unless its name ends in `?`.** The marker is stripped before Harbor checks the server schema, so `reference_image_base64_1?` addresses the bare `reference_image_base64_1` property. An optional slot that is absent (including JSON `null`) is skipped; an explicitly empty string is still refused as `ErrEmptyArtifactID`.
