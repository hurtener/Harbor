...
- **`tools.list`** — until the per-tool annotator backend ships (Phase 178, negotiate via `Accepts(tool_annotations)`), `filter.oauth_statuses` / `filter.approval_policies` loud-reject with `invalid_request`, and the response carries `aggregates_partial: true` with the annotator-backed counters (`active` / `pending_approval` / `awaiting_oauth`) zeroed — render them "unavailable," never a real-looking 0. Only `aggregates.total` is authoritative in that state.
