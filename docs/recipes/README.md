...
| [Embed and retrieve](embed-and-retrieve.md) | The Phase 84d `Embedder` à la carte — `embeddings.Open` + `Embed` + `Cosine` over your own corpus, plus opt-in semantic skill retrieval (`skill_search`); session memory uses cumulative checkpoints, not a vector index. |
