...
- [x] `steering.RunSpec` gains `CompletionHook *CompletionHookSpec` (tool name + timeout + optional agent_id metadata); `RunLoop.Run` fires it exactly once at the terminal boundary (deferred over named returns) for ALL exits — `FinishGoal`, `FinishNoPath`, `FinishConstraintsConflict` (incl. REJECT + pause-timeout), `FinishCancelled` (incl. cancel-while-paused), and error returns (mapped to `error`, or `cancelled` when `errors.Is(err, context.Canceled)`) — and never on pause or mid-run. A nil `CompletionHook` is byte-identical to today's behaviour. (Outcome table + fire tests in `completion_test.go`; the nil-hook byte-identical case pinned.)
- [x] The hook dispatches through `spec.ToolExecutor.ExecuteDecision` with a synthetic `planner.CallTool` under `context.WithTimeout(context.WithoutCancel(runCtx), timeout)`; the run's identity quadruple is observable at the receiving tool. The dispatch does NOT invoke `spec.OnToolDispatched`, does NOT append a trajectory step, and does NOT alter `ToolCallsSeen` / `tool_count` (D-274 counters are planner-loop semantics; asserted by `TestRun_CompletionHook_D274CountersUntouched` + the E2E identity-at-the-sink assertion).
- [x] A hook dispatch error (unknown tool, executor error, timeout, nil `ToolExecutor` with a configured hook) leaves `(fin, err)` untouched, emits `run.hook_failed` (SafePayload: identity, tool, outcome, error class — no transcript, no raw error text quoting caller data) and Warn-logs; success emits `run.hook_dispatched` (identity, tool, outcome, duration, transcript bytes + entry count). Both types registered in `internal/runtime/steering/events.go` from `init()`; the events exhaustiveness test extended (`TestRunHookEventTypes_Registered`, `TestRunHookPayloads_AreSafePayload`).
- [x] `steering.RunCompletionPayload` (`format_version: 1`) assembles the ordered transcript — initial goal, steering USER_MESSAGE + REDIRECT entries in arrival order with step indices, assistant preambles + compact tool lines, final answer — from the trajectory + the new per-run stack-local steering accumulator; a golden test pins the JSON encoding (`TestBuildRunCompletionPayload_Golden`); an ordering unit test drives interleaved user messages + redirects across steps (`TestAssembleTranscript_InterleavedAcrossSteps`).
- [x] Config: `runtime.hooks.run_completion.{tool, timeout}` in `internal/config` (validated; `examples/harbor.yaml` + `dev.yaml` + `serve.yaml` + `docs/CONFIG.md` updated) + the `agentcfg.ConfigPayload.Hooks` section (revision round-trip, section-merge preservation, diff hooks arm, rollback — `TestStateStore_Hooks_RoundTrip_SectionMerge_Diff_Rollback`; a wire-edge set-time validation arm rejects a negative `timeout_ms` with `ErrInvalidHooks` → 400, `TestSetRevision_Hooks_NegativeTimeoutRejected`) + `projection.ActiveRunCompletionHook` with the agentcfg-over-yaml precedence pinned by `TestActiveRunCompletionHook_Precedence`, called by both the dev binary and the devstack twin (§17.6). The yaml half is the shared `projection.RunCompletionHookFromConfig`, ALSO applied by the embed path: `Stack.RunOnce` resolves the hook from the stack's static config with a `WithCompletionHook` RunOption for per-call override / explicit-nil disable (`TestRunOnce_CompletionHook_*` — yaml fires at the sink; option overrides yaml; nil disables; no-config-no-option byte-identical), so embed / foreground / background run types are covered uniformly.
- [x] Wire lockstep: `types.AgentConfigHooks` (+ `AgentConfigHooksDiff`) + the `AgentConfigDiff` hooks arm land in `internal/protocol/types/agentconfig.go`; the TS mirror is hand-updated; `make protocol-ts-gen` regenerates the manifest and `make protocol-ts-gen-check` passes; `make protocol-docs-gen` regenerates the Protocol reference; no new method, no version bump (additive types only).
- [x] §13 primitive-with-consumer integration test (`test/integration/phase150_run_completion_hook_test.go`): a run with a completion hook targeting a real fixture tool (in-proc registered sink) receives the full ordered transcript INCLUDING a steering-injected mid-run USER_MESSAGE (injected through the REAL steering inbox from the work tool); identity propagation asserted AT THE RECEIVING TOOL; real drivers (real RunLoop + `dispatch.NewToolExecutor` + inmem bus/state/artifacts + deterministic planner); `-race`. (The MCP-sourced leg is a documented §4.3 deviation — the in-proc sink exercises the same `ExecuteDecision` path; an MCP leg adds transport plumbing without exercising a distinct hook code path.)
- [x] Failure-mode legs: (a) hook tool errors → run outcome unchanged + `run.hook_failed` observed on the bus (`TestE2E_Phase150_HookErrorLeavesOutcomeUnchanged`); (b) a CANCELLED run → hook still fires with `outcome: cancelled` under the detached bounded ctx, identity preserved, sink ctx not cancelled (`TestE2E_Phase150_CancelledRunFiresWithCancelledOutcome`).
- [x] Concurrency: N≥10 concurrent runs with hooks against ONE shared RunLoop + executor + sink under `-race` — no cross-run transcript bleed (per-run sentinel goals asserted at the sink), in both the unit (`TestRun_CompletionHook_ConcurrentNoBleed`, N=24) and integration (`TestE2E_Phase150_ConcurrentNoBleed`, N=16) suites. The RunLoop's existing D-025 suite is unchanged (the hook adds no struct-level state).
- [x] `scripts/smoke/phase-150.sh` flips from skeleton to real assertions (13 OK / 0 FAIL).
- [x] `docs/decisions.md` D-280 flipped to Shipped; `docs/glossary.md` terms verified accurate; `docs/plans/README.md` row + detail block flipped to Shipped (v1.10); the RFC §6.17 subsection was already applied with the plans PR.
...
- `projection.ActiveRunCompletionHook(ctx, reg, agentID, id, yamlDefault) (*steering.CompletionHookSpec, bool, error)` + `projection.RunCompletionHookFromConfig(rc) *steering.CompletionHookSpec` (the shared yaml half)
- `assemble.WithCompletionHook(spec *steering.CompletionHookSpec) RunOption` (the embed per-call override / explicit-nil disable; unset = the stack's static config applies)
...
- [ ] `make preflight` — NOT run locally (per the dispatch contract; CI runs the gate; the commit uses `HARBOR_PREFLIGHT_SKIP=1` justified in the PR body). `scripts/smoke/phase-150.sh` passes standalone (13 OK / 0 FAIL) and the full static smoke battery was run.
- [x] `make check-mirror` passes
- [x] All cross-references (`RFC §X.Y`, `brief NN`) resolve
- [x] Coverage on touched packages ≥ stated target (`internal/runtime/steering` ≥ 80%; agentcfg / projection / config no regression)
- [x] If multi-isolation paths changed: cross-session isolation test passes (the N≥10 no-bleed tests double as this — transcripts are per-run stack-local state; each run's payload carries its own identity + goal)
- [x] **Concurrent-reuse:** the hook adds NO struct-level state to the RunLoop (all per-run hook state — the steering accumulator, initial goal, start time — is stack-local on the run goroutine); the existing RunLoop D-025 N≥100 suite is unchanged, and the hook surface is covered by the N=24 unit + N=16 integration no-bleed tests under `-race`.
- [x] **Integration test:** `test/integration/phase150_run_completion_hook_test.go` consumes 53 + 83i + 92a + 132, wires real drivers end-to-end, asserts identity propagation at the receiving tool, covers two failure modes (hook error, cancelled run), and runs under `-race`.
- [x] If new vocabulary: glossary verified accurate (terms landed with the plans PR)
- [x] If a brief finding was departed from: N/A — none departed from
