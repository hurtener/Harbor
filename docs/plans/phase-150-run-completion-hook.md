...
- [x] Config: `runtime.hooks.run_completion.{tool, timeout}` in `internal/config` (validated; `examples/harbor.yaml` + `dev.yaml` + `serve.yaml` + `docs/CONFIG.md` updated) + the `agentcfg.ConfigPayload.Hooks` section (revision round-trip, section-merge preservation, diff hooks arm, rollback — `TestStateStore_Hooks_RoundTrip_SectionMerge_Diff_Rollback`) + `projection.ActiveRunCompletionHook` with the agentcfg-over-yaml precedence pinned by `TestActiveRunCompletionHook_Precedence`, called by both the dev binary and the devstack twin (§17.6).
