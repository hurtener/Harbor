...
- **DEFERRED (Wave 10 audit DEFER-1; tracked in [#94](https://github.com/hurtener/Harbor/issues/94)):** the carrier-into-transport wiring was NOT included with Phase 55 — the Wave 10 audit surfaced that the carriers ship as standalone helpers without a production consumer in the repo today. The first production consumer lands with Phase 64's `harbor dev` server bootstrap (HTTP transport) plus a future MCP-side phase (`_meta` adoption + the env carrier on stdio spawn). See issue #94 for the action-item breakdown.
