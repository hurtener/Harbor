...
- **Conformance:**
  - `pause.list` is registered in the Phase 62 Protocol conformance matrix (`internal/protocol/conformance/conformance.go`'s method matrix) so the matrix-exhaustiveness check at the top of `RunSuite` stays satisfied — a new method that is absent from the matrix fails the suite (D-080 lockstep).
  - **§17.6 deferral (D-132 / Wave 13 §17.5 W9):** the `pause.list` happy-path AND malformed-request *scenarios* are `t.Skip`'d in the conformance suite with an explicit, observable reason. `pause.list` is a read-only snapshot that routes through its OWN HTTP handler (`POST /v1/pause/list`), NOT the REST `ControlSurface` the conformance Stack drives — the same architectural shape as the `search.*`, posture, `topology.snapshot`, `artifacts.*`, and `memory.*` methods, all of which the conformance Stack skips for the identical reason. Wiring a live `pause.list` handler into the conformance Stack is a suite-wide harness extension (it must equally re-home search / posture / topology / artifacts / memory) deferred to the Phase 80 conformance-surface extension. Until then `pause.list` is exercised end-to-end by `pause_list_handler_test.go` + `test/integration/pause_list_test.go` (the §13 primitive-with-consumer binding test). The original plan text ("happy-path + malformed scenario in the same PR") yielded to this checkpoint finding: the matrix *entry* lands in lockstep; the *scenario bodies* land with the Phase 80 harness extension.
- **Concurrency / leak:**
...
- **Scope-claim handling consistency** (resolved per audit R7 / N2): 72e + 72a + 72c + 73c + 73e all use `auth.ScopeAdmin` from D-079's closed two-scope set (`ScopeAdmin` + `ScopeConsoleFleet`). No new `events.crosstenant` or `search.crosstenant` scope is minted (Phase 72's Non-goals explicitly forbid a third scope). Alignment verified post-audit.
...
- [ ] Phase 62 conformance matrix registers the `pause.list` method entry in the same PR (D-080 lockstep — matrix exhaustiveness check fails the suite otherwise). The happy-path + malformed *scenario bodies* are `t.Skip`'d with an observable reason and deferred to the Phase 80 conformance-Stack harness extension — see the §"Conformance" §17.6 deferral note (D-132 / Wave 13 §17.5 W9).
