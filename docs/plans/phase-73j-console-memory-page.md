...
below is **rejected on sight**. The Memory page is a catalog surface with a
selected-item detail rail; it mounts inside the shared app shell and clears
the §5 depth bar like every other page.

The page MUST:

- **Route under `(console)/`.** The page lives at
  `web/console/src/routes/(console)/memory/` and is served at `/memory` with
  **no `/console/` URL prefix** (the `(console)` route group is a
  ...
  `(console)/memory/[id]/` and are served at `/memory/<id>`. All inter-page
...
- **Use the shared `components/ui/` inventory.** It composes the cross-page
  ...
  page-specific components go in `components/memory/`.
