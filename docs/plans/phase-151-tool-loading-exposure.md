...
- **Integration:** `test/integration/agentcfg_loading_exposure_test.go` — real drivers on every seam (StateStore-backed agentcfg registry, inmem bus/state, real Protocol service + wire handler, real catalog, the shared run-start projection, the real MCP stdio fixture `cmd/harbor-mcptest-stdio` per §17.8): `set_tool_exposure` flips the fixture's `echo` tool to deferred → next projection's prompt-time list excludes it, `tool_search` surfaces it, `Resolve` returns it; flip back → visible; invalid mode → loud 400, revision chain unchanged; identity propagation end-to-end; cross-agent + cross-tenant isolation. All `-race`. (The disabled-stronger-than-deferred leg — a disabled tool absent from BOTH `List` and `Resolve` while a deferred tool stays resolvable — is pinned by the projection test `TestActivePlannerCatalogView_LoadingOverride_ExclusionStaysStrongerThanDefer`; the no-dispatch-when-disabled behaviour is the shipped 92d contract, already pinned end-to-end by `test/integration/agentcfg_mcp_policy_test.go` — not re-proved here.)
...
- Live (using the preflight-exported `HARBOR_DEV_TOKEN`): a fixed smoke-owned agent id; `tools.list` resolves a real catalog tool and `tools.describe` pins its boot-effective `loading_mode` (SKIPping the describe/flip round trip when the build's config registers no tools — `examples/dev.yaml` ships empty `tools.built_in` / `tools.mcp_servers`); `agent_config.set_tool_exposure` with a `tool_loading_modes` flip → re-describe with `agent_id` asserting the effective `loading_mode` flipped; a request with `"loading":"sometimes"` → 400 `invalid_request` AND no revision recorded (asserted via `agent_config.get` — this leg needs no real tool, so it always runs); restore (set exposure without the override) → describe reports the original mode. Unauthenticated POST stays non-200 (the 92d gate assertion, re-checked). **Deviation from the original sketch (§4.3):** the sketch resolved the dev agent id via `agents.list`; shipped, the smoke uses a fixed id instead — `agent_config.*` / `tools.describe`'s `agent_id` is an opaque per-agent KEY into the agentcfg registry (never an Agent-Registry lookup, never an isolation principal — CLAUDE.md §6 clarifying note), so an `agents.list` round trip adds nothing and would false-SKIP on a build with no self-registered agents.
