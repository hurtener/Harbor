...
| `Fence_DropsLateEventsAndEmptiesHistory` (incl. Unfence restore + cross-session non-bleed) | `TestInmem_Fence_DropsAndEmptiesHistory` inmem_fence_test.go:16 | `TestDurable_Fence_DropsLateEventsAndEmptiesHistory` durable_fence_test.go:18 |
...
- [ ] Every row of the fold mapping table is realized: the suite scenario exists with the pinned name, its assertions cover the old test's assertions (reviewed row-by-row against the table), and the old duplicated test functions are DELETED from the driver packages (smoke grep-absent). `inmem_fence_test.go` and `durable_fence_test.go` are removed entirely.
- [ ] The identity-scoping scenarios (`Publish_IdentityMandatory_Rejected`, the three empty-triple rejections, `Subscribe_CrossTenant_Isolation`, `Replay_CrossIdentity_Isolation`, the fence non-bleed) run unconditionally for both drivers (§6 rule 10) — no skip path exists in the suite for them.
- [ ] Capability presence is asserted fail-loud: a factory whose default bus does not type-assert to `events.Replayer` + `events.HistoryReplayer` + `events.Fencer` fails the suite with a named error — no capability-gated skips (§4.4).
- [ ] Zero production code change: the PR's diff touches only `internal/events/conformancetest/`, `_test.go` files under `internal/events/drivers/`, `scripts/smoke/phase-147.sh`, and docs (plans/decisions/glossary/README). Asserted by review + the smoke's grep legs.
- [ ] `go test -race -count=1 ./internal/events/...` and `-count=2` are green (the §17.6 test-idempotency lesson — folded scenarios must not depend on left-over state), and no suite scenario synchronizes via `time.Sleep` (§17.4 — channel reads with bounded timeouts only).
- [ ] Statement coverage on `internal/events/drivers/inmem` and `internal/events/drivers/durable` does not regress from the pre-fold baseline (recorded in the PR description).
- [ ] `scripts/smoke/phase-147.sh` flips from skeleton to real assertions (the unit-test leg + the folded-names-gone grep + the both-drivers-invoke grep + the no-concrete-driver-import grep).
- [ ] D-277 lands in `docs/decisions.md`; "events conformance suite" lands in `docs/glossary.md`; the master plan row + detail block land in `docs/plans/README.md` — same PR.
...
- [ ] Coverage on touched packages ≥ stated target (no regression on either driver package; baselines recorded in the PR)
- [ ] If multi-isolation paths changed: cross-session isolation test passes (no production isolation path changes; the suite's isolation scenarios themselves are the strengthened gate)
...
- [ ] If new vocabulary: glossary updated ("events conformance suite")
- [ ] If a brief finding was departed from: justified above + decisions.md entry filed (None departed from)
