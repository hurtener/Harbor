...
- [x] Statement coverage on `internal/events/drivers/inmem` and `internal/events/drivers/durable` does not regress from the pre-fold baseline (recorded in the PR description). inmem 93.3%→93.8%, durable 88.3%→88.6% — both improved slightly, no regression.
...
- [x] Coverage on touched packages ≥ stated target (no regression on either driver package; baselines recorded in the PR). inmem 93.3%→93.8%, durable 88.3%→88.6%.
