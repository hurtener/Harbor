...
below is **rejected on sight**. The Events page's live stream is its primary view in the §5 depth bar; it still mounts inside the shared shell and routes its async state through the four-state `<PageState>`, keeping Disconnected distinct from a stream Error.

The page MUST:

- **Route under `(console)/`.** The page lives at
  `web/console/src/routes/(console)/events/` and is served at `/events` with
  **no `/console/` URL prefix** (the `(console)` route group is a
  ...
  `(console)/events/[id]/` and are served at `/events/<id>`. All inter-page
...
- **Use the shared `components/ui/` inventory.** It composes the cross-page
  ...
  page-specific components go in `components/events/`.
