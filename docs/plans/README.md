...
|233b | Signed OAuth MCP capability registration (HA-50, D-401): production-safe boot-authorized closed descriptor registration with durable tenant-scoped JTI and paired-removal recovery, pair-owned provider/catalog-only dispatch, a committed agent activation fence, canonical HTTPS URL bytes/sink, and bounded boot/run-start durable reconciliation | agentcfg + Protocol + tools/auth + MCP serve + config | §4, §5.5, §6.4, §6.11, §6.16 | 233 | 85–90% | Shipped (v1.26) |
...
- **Ordering:** 233a and 233b are independent after 233; both gate 234. Phase
  ...
  and 234.
- **Decision:** D-401. **Status:** Shipped (v1.26).
