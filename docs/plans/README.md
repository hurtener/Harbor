...
|101 | GitHub Actions Node 24 modernisation          | .github/workflows    | §12         | 81                    | n/a  | Pending (V1.1.x) |
|102 | Godoc hygiene — strip internal phase jargon   | internal/ + cmd/     | §1, §12     | (none hard)           | n/a  | Pending (V1.1.x) |
|103 | GitHub Pages docs site (Dockyard parity)      | docs/site + workflows| §1, §7, §12 | 102, 85k              | n/a  | Pending (V1.1.x) |
...
|111d| Skills canonical surface + ingestion (builtin→Phase-38 delegation; `harbor skill import`/`rm`; Directory disposition decision) | internal/skills + internal/tools/builtin + cmd/harbor | §6.7, §8 | 37, 38, 39, 40, 41, 107c | 85% | Pending (V1.1.x) |
|111e| Trajectory compression consumer (LLM-backed `planner.Summariser` + RunLoop `MaybeCompress` + `token_budget` wiring) | internal/llm/summarizer + internal/planner + internal/runtime/steering + cmd/harbor | §6.2, §6.5 | 46, 35, 107, D-192 fix | 85% | Pending (V1.1.x) |
|111f| Telemetry assembly + approval-gate authorizer seam (`telemetry.New` in production; `BridgeBusToTracer`; protocolauth out of approval) | internal/telemetry + internal/tools/approval + internal/runtime/steering + cmd/harbor | §6.14, §6.4, §5.1 | 03, 04, 05, 31, 55, 56, D-192 fix | 85% | Pending (V1.1.x) |
...
- **111a — Governance enforcement assembly (D-198 reserved).** `governance.SetFactory`'s only caller is a test; populated `governance.identity_tiers` drives the posture provider only — clean validation, zero enforcement. Ships exported `governance.NewSubsystemFromConfig(cfg, store, bus)` (Compound in the documented MaxTokens→RateLimiter→CostAccumulator order; nil Subsystem on empty tiers preserving the D-044 latent default), called via `SetFactory` from the production assembly when tiers are non-empty; consumes 110c's `governance.ConfigFromOperator`; documents `governance.Wrap` as the multi-runtime headless escape (SetFactory-vs-per-Open evaluated + decided); removes the Wave A posture-only boot warning; E2E proves a configured tier actually rejects/limits (cost + rate + MaxTokens each exercised). RFC §6.15, §6.5, §6.11. Deps: 32, 36a, 36b, 110c (soft). See `docs/plans/phase-111a-governance-enforcement-assembly.md`.
- **111b — Tool-OAuth completion leg (D-199 reserved).** The pause producer is live but `Provider.CompleteFlow` has zero callers and no route anywhere exchanges `(state, code)` — the "Harbor Protocol callback handler" the godocs name does not exist. Ships exported `auth.CallbackHandler` (state→`PendingFlow`→`CompleteFlow`→typed error mapping), mounted by `harbor dev` at `/v1/tools/oauth/callback` and mountable headless; the full choreography E2E (gated tool → `pause.requested` → `InitiateFlow` URL → simulated callback → resume → run re-enters with the token) against a test OAuth server; ships the OAuth section of `docs/recipes/steer-and-resume-a-run.md`. Closes the `InitiateFlow`/`CompleteFlow` primitive-without-consumer pair (§13). RFC §6.4, §3.3, §6.3. Deps: 30, 50, 31, the D-192 steering fix. See `docs/plans/phase-111b-tool-oauth-completion.md`.
- **111c — Durable pauses + pause lifecycle (D-200 reserved).** `WithCheckpointStore` has zero production consumers (both assemblies construct the Coordinator storeless with the StateStore in scope); `requestPause` persists `Trajectory: nil`; no pause GC exists — `DecisionTimeout` has no producer and cancel-while-paused orphans records forever. Threads the run's Trajectory into `requestPause`; wires `WithCheckpointStore(stateStore)` in both assemblies; pause→new-Coordinator-over-same-store→Resume→trajectory-restored E2E (+ the §11 `ErrUnserializable` fail-loud test); ships `WithMaxParkDuration` + an exported sweeper over the existing List/Resume surface emitting `pause.resumed` with `DecisionTimeout` — its first producer — started by the assemblies. RFC §3.3, §6.3, §6.11. Deps: 50, 51, the D-192 steering fix. See `docs/plans/phase-111c-durable-pause-lifecycle.md`.
- **111d — Skills canonical surface + ingestion (D-201 reserved).** Three intertwined gaps: the rich Phase-38 planner tools (capability filter, redaction, budgeter) + Phase-41 generator are registered nowhere while production registers a thinner parallel builtin implementation (the §13 two-implementations smell); Skills.md ingestion has no shipped path (importer test-only; the SKILL.md documented fictional `harbor skill import`/`rm` verbs); the Phase-39 Directory has only test consumers. Resolution: builtin `skill_search`/`skill_get` delegate to the Phase-38 handlers (duplicate bodies deleted; filter/redaction/budgeting re-homed onto production); `skill_list` + `skill_propose` (default-off) register through the same carrier; `harbor skill import`/`rm` ship over exported `importer.ImportAndStore` (+ §18 SKILL.md restoration in the same PR); the Directory gets an explicit owner decision before implementation — plan recommendation: wire `Directory.View` as the `<skills_context>` producer. RFC §6.7, §8. Deps: 37–41, 107c. See `docs/plans/phase-111d-skills-canonical-surface.md`.
- **111e — Trajectory compression consumer (D-202 reserved).** `planner.Summariser` has only test implementations; `MaybeCompress` has zero call sites; `Budget.TokenBudget` is dead on every path — while the consumer half (the React prompt's `Summary != nil` branch) is already wired. Ships a real LLM-backed `TrajectorySummariser` (in `internal/llm/summarizer`, distinct from the unrelated `memory.Summarizer` — do not conflate), the `MaybeCompress` integration in `steering.RunLoop`'s step loop gated on `TokenBudget > 0`, the `planner.token_budget` config → `RunSpec.Budget` production wiring, and the godoc un-dormanting. Long-trajectory E2E: compression fires, the prompt shrinks, the run completes correctly on summary-carried context. Scoped tight: one compression per run, no auto-cascade. RFC §6.2, §6.5. Deps: 46, 35, 107, the D-192 steering fix. See `docs/plans/phase-111e-trajectory-compression-consumer.md`.
- **111f — Telemetry assembly + approval-gate authorizer seam (D-203 reserved).** Two halves. Telemetry: `telemetry.New` (redactor-mandatory, identity-attribute, bus-paired Logger) has zero production callers — cmd boots bare slog; `engine.WithRunErrorHandler` has no production caller; metrics got `BridgeBusToMetrics` but traces got no bridge and `NewTracer` is never constructed despite the blank-imported exporters. Wires `telemetry.New` + the RunErrorHandler into the production assembly, ships `telemetry.BridgeBusToTracer` symmetric with metrics, and ships `docs/recipes/observe-an-embedded-runtime.md`. Approval: `ResolveApproval` hard-requires `internal/protocol/auth` scopes and the runtime's own steering bridge self-elevates to pass its own gate — wire-layer auth vocabulary in an in-process control path. Ships an injected authorizer seam on `GateDeps` (runtime identity/control-scope default; protocolauth via a server-side adapter), removes the protocol/auth import from `internal/tools/approval`, and records the direction rule: runtime may import protocol TYPES, never protocol auth/methods/transports. RFC §6.14, §6.4, §5.1. Deps: 03, 04, 05, 31, 55, 56, the D-192 steering fix. See `docs/plans/phase-111f-telemetry-assembly-approval-seam.md`.
