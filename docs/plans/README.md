...
- **Subsystem:** internal/events (the aggregator, the `HistoryReplayer` fan-in,
  the shared conformance suite, both drivers), internal/protocol/transports/stream
  (the aggregate handler threads the server-derived `widened` decision).
...
- **Deps:** 72a (`events.aggregate` + the aggregator), 162 (`events.list` /
  ...
  durable log), 125 (`state.history` / `HistoryReplayer` seam; D-254).
- **What it delivers:** the instance AND its class in one phase (the D-283
  ...
  `audit.admin_scope_used` on the widened path; a generous cap keeps memory
  equal to today's whole-ring materialization; a window past the cap fails loud
  `ErrAggregateWindowTooLarge`, never a silent undercount), threading the
  handler's server-derived `widened` decision (D-299) as a Go input — never a
  wire field — which also fixes a latent audit-integrity bug (the hardcoded
  `Admin:true` would emit `admin_scope_used` for a non-admin own-session read).
  ...
  scenario + a four-method parity leg (`events.aggregate`/`events.list`/
  `events.subscribe`/`state.history` against every registered driver → same
  answer or same named sentinel) + a registry gate (a self-registered driver
  with no conformance run fails the build). The thesis, made executable: a
  driver difference may change WHAT a method returns (depth via `truncated`, an
  observed horizon) but NEVER WHETHER it works — differences stay DATA/named
  sentinels, never a 500, never normalized. ZERO-WIRE (a manifest diff is a red
  flag). §17.1 integration test: `events.aggregate` 200-not-500 on durable over
  a real StateStore, identity propagation, ≥1 failure mode. See
...
- **What it delivers:** an aggregate bucket is a bag of scalars with NO tenant
  ...
  admin/console:fleet, server-derived per D-299) and ONLY for tenants the caller
  is already entitled to; an unelevated caller gains attribution for nothing new.
  The per-bucket invariant `Σ counts_by_tenant == counts` proves it is a pure
  re-projection, not a looser read path. Makes the isolation boundary
