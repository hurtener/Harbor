...
> **83-band runtime consumer gap (open follow-up).** 83a/b ship wired through the planner factory and reach `harbor dev` operators directly; 83c/d/e ship the per-run primitives (`RepairCounters`, `PlanningHints`, `MemoryBlocks`, `SkillsContext`, reasoning trace round-trip) but `cmd/harbor/cmd_dev_runloop.go` does not yet populate them. Tracked at **#208** — likely sibling phase `83f` (runtime consumers); flagged by the Wave 15 §17.5 checkpoint audit.
