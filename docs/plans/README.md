...
|112a| The public SDK facade (`sdk/` alias-based re-export tree per RFC §3.6) | sdk/ (new top-level) | §3.6, §1 | 110a-d, 111a-f, D-204 | n/a | Pending (V1.2) |
|112b| External consumers on the facade + the external-module compile gate (scaffold templates, harbortest vocabulary, recipes/README, the standing gate) | cmd/harbor/scaffold + harbortest + docs | §3.6, §8 | 112a | n/a | Pending (V1.2) |
...
- **112a — The public SDK facade (D-205 reserved).** The `sdk/` tree of alias-based re-exports per RFC §3.6's inventory; facade-integrity test compiles a headless assembly via `sdk/` imports only; `sdk/drivers/prod` parity with the internal aggregator; AGENTS/CLAUDE §3 amendment. No moves, no mechanism. See `docs/plans/phase-112a-sdk-facade.md`.
- **112b — External consumers + the compile gate (D-206 reserved).** Scaffold templates emit `sdk/` imports (tool-declaring output compiles as an external module — the audit's headline external break, now gated); harbortest vocabulary externally satisfiable via the aliases; consumer-facing recipes + README flipped; the standing external-module compile smoke. See `docs/plans/phase-112b-external-consumers.md`.
