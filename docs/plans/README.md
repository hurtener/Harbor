...
| 84e| Semantic memory consumption in the run loop (`SearchTurns` recall → `<read_only_external_memory>`; opt-in via 84d) | internal/runtime/runctx + internal/memory + internal/config + cmd/harbor + harbortest | §6.2, §6.5, §6.6 | 84d, 83d, 83f, 110b, 110c, 107c | 90% | Pending (V1.1.x) |
...
- **84e — Semantic memory consumption in the run loop (D-211 reserved).** Closes the gap 84d (D-191) left by design: `MemoryStore.SearchTurns` shipped with store/SDK consumers only — nothing in the run loop calls it, so the agent never semantically recalls earlier conversation turns and the planner prompt's memory injection (the 83d path) stays rolling-summary-only. 84e makes the run loop, when `memory.retrieval: semantic` is on (the ONLY switch — no second knob), search the session's embedded turns with the task query and inject the top-k recalled turns into the prompt's `<read_only_external_memory>` tier — the `planner.MemoryBlocks.External` slot brief 13 §2.3 defines as "retrieved before this run," nil on every production path since 83d, so the planner gains zero new surface and recall inherits the UNTRUSTED framing for free. Composes with `rolling_summary` (the Conversation tier is byte-untouched); the 84b golden default-parity posture applies — mode off → byte-for-byte prompt parity, zero embedder traffic. The fetch+recall step is promoted to ONE home, `runctx.FetchMemoryBlocks` (the 110b pattern), consumed by both `cmd/harbor`'s runOne and the devstack mirror — the per-step `GetLLMContext` mirror copies collapse. Knobs ride the existing memory block (`retrieval_top_k` reused; new `retrieval_min_score` similarity floor, default 0.0) via a 110c-shaped `memory.RecallFromConfig` exporter with field-parity test. Degradation posture is fail-loud per the band: a recall error fails the run (`runtime_fetch_error`, LLM never called) — never a silent fall-back to rolling-summary-only (§13). Recalled turns are text-only and per-turn capped (D-026; the LLM-edge guard stays the backstop); the 111e `planner.token_budget` interaction is bounded by construction (injection messages, not trajectory). Records the deferred sibling: a `memory.search` Protocol method must precede any Console memory-search page (D-062 ordering) — parked for post-109 planning. RFC §6.2, §6.5, §6.6. Deps: 84d, 83d, 83f, 110b, 110c, 107c (111e soft). See `docs/plans/phase-84e-semantic-memory-runloop.md`.
