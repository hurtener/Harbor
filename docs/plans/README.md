...
- **Deps:** 163 (HA-14 / D-296 — the retention block + `RetentionProvider` this
  phase extends), 118 (D-223 lockstep). Composes with the HA-21 events-fold work
  (reachable vs verifiable — orthogonal, either order).
- **What it delivers:** `runtime.health`'s `retention[]` block (HA-14/D-296)
  ...
  it, else stated inline here). Composes with HA-21: HA-21 makes the session-less
