...
|269 | Retained session execution context | runctx + assembly + StateStore | §6.2, §6.9, §6.11 | 15, 16, 17, 246, 268 | embedded request + inmem/SQLite + 128-session race; full gates pending | In progress — embedded terminal retention only |
...
- **v1.29.4 correction:** durable turn materialization now advances through a
  ...
  global `state_records` prefix scans on an idle runtime. Concurrent final
  fence filtering preserves the page's pre-filter overflow proof, so a
  truncated page cannot promote its checkpoint past a later canonical event.
- **v1.29.5 correction:** the turns materializer's lost-wake fallback for
  ...
  seconds. Durable event watches remain the fast path, and the bounded
  fallback still catches up when the source watermark is ahead or a deferred
  terminal snapshot needs reconciliation; an idle durable runtime now performs
  two source-watermark reads per minute instead of thirty.
...
- **Decision:** D-425. **Status:** Shipped (v1.28; v1.29.4 and v1.29.5
  corrections shipped).
...
- **v1.29.4 correction:** rollup lost-wake polling now compares the cheap
  source watermark with the durable projection checkpoint before opening a
  bounded source page. A current idle projection performs no global
  event-head scan; stale and catching-up projections retain the existing
  bounded replay and explicit completeness semantics.
...
- **Decision:** D-426. **Status:** Shipped (v1.28; v1.29.4 correction
  shipped).
...
### Phase 253 — v1.29.3 offline legacy-head integrity repair (HA-69 extension)

- **Subsystem:** durable event recovery and StateStore repair contract; offline
  Harbor CLI; operator documentation; hosted real-PostgreSQL acceptance.
- **RFC:** §4.3, §6.11, §6.13, §8, §12. **Deps:** 16, 57, 251, 252; D-294,
  D-305, D-398, D-431, and D-432.
- **What it delivers:** D-433 — an offline `harbor events
  repair-legacy-heads` surface that defaults to content-free inspect/dry-run,
  requires explicit freeze/drain acknowledgement for apply, and rejects
  transaction-pooled/PgBouncer or non-direct PostgreSQL mutation endpoints.
  Exact duplicate references are validated against immutable entry bodies,
  storage and event identity, sequence, kind, type, metadata, and checksums;
  only redundant references and duplicate projections are removed through the
  shared StateStore conditional-write contract. Receipts are durable and
  content-free, and response-loss/repeated apply is idempotent. Ambiguous or
  malformed data remains fail-closed, including the session-scoped `RunID=""`
  storage-key exception documented by D-432.
- **Evidence:** Focused in-memory/SQLite/Postgres conformance, cancellation,
  CAS-race, response-loss, concurrent-attempt, CLI no-runtime-boot, and exact
  direct-5432 acceptance passed. Hosted candidate run `32600083120` and
  documentation run `32600083113` completed successfully; the exact hosted
  real-Postgres repair selector and PASS-line guard are retained. The
  immutable annotated `v1.29.3` tag object is
  `eeae1f44f4fb7d862f581f9cbbabb40a7827146a` and peels to
  `dabbcff4f0bbadf7d5710d0b8844b639512ca4ac`; release workflow
  `32602553267` succeeded, publishing [13 release assets](https://github.com/hurtener/Harbor/releases/tag/v1.29.3)
  with verified aggregate and six sidecar checksums and six attestations. The
  ...
  `Origin.Ref=refs/tags/v1.29.3`. The post-tag scaffold pin/golden cleanup
  is complete. Local preflight is not run and no downstream fleet repair or
  cutover is claimed.
- **Decision:** D-433. **Status:** Shipped (v1.29.3; immutable tag, release,
  provenance, checksums, hosted gates, and post-tag scaffold pin verified);
  extends HA-69 without allocating a new ask identifier.
...
- **Evidence:** Focused grant, Bifrost, retry, receipt, in-memory, SQLite, and
  ...
  implied by the plan. Implementation PR #738 was merged at
...
- **Evidence:** Focused catalog/adapter/CLI race tests, vet, static smoke,
  ...
  implied by the phase. Implementation PR #738 was merged at
  `d9bf28fe703e10eb9f995657f4ac52949aa57e04` (tree
  `72f8093049a3f7bc952d8d3e0decdd8d02ea7744`). Hosted candidate run
  `32670321270` completed successfully on PR head
  `1da7845326088e451bcf19970136a62b8e274e5a` (the same tree), including the
...
### Phase 256 — Public external-grant SDK and explicit runtime-default route (HA-70 compatibility extension)

- **Subsystem:** public SDK LLM/grant/receipt facades, external-grant wrapper,
  receipt outbox, runtime assembly, and LLM configuration.
- **RFC:** §6.5, §6.11, §6.15. **Deps:** 33, 36a, 57, 91, 93, D-025,
  D-333, D-334, D-434, and D-435.
- **What it delivers:** D-436 — a public, nameable grant/config/signer/
  verifier/receipt-delivery contract, canonical receipt JSON/hash and
  receipt-to-grant derivation validator, and an explicit signed route mode.
  `coordinator_bound` preserves v1.30.0; opt-in `runtime_default` uses the
  boot-configured provider/model and records actual route facts without
  carrying coordinator credentials. Mixed claims and cross-provider fallback
  fail closed before provider invocation; retry/downgrade coordinates remain
  receipted and response-loss identity remains stable. No Protocol version,
  HTTP delivery endpoint, or presentation/policy layer is added.
- **Evidence:** The focused LLM/config/assembly/SDK suites, assembled
  runtime-default first consumer, external-package SDK consumer, adversarial
  receipt derivation tests, race/vet, static smoke, and documentation gates
...
- **Decision:** D-453, D-454, D-455. **Status:** Shipped in Harbor v1.31.0;
  exact post-merge main CI run `33297306154` and release workflow `33299904384`
  completed successfully. Downstream deployment and acceptance remain pending.
  See `docs/plans/phase-266-ordered-observability-event-path.md`.
...
- **Boundary:** both agents must be in the same verified tenant/runtime and in
  ...
  unchanged.
- **Evidence:** exact body/hash vectors, strict typed routes, source/target
  ...
  shipped drivers. The annotated `v1.31.0` tag object `dc009bb5` peels to
  `f6d87b27`; exact main CI and release workflow completed successfully with
  13 published assets. PR #765 then corrected explicit-empty reconciliation
  and merged as `8bc070c1`; release-hygiene PR #766 merged as `eabdbb67`.
  Exact main CI run `33308171578` and v1.31.1 release workflow `33311306845`
  completed successfully with 13 assets. PR #768 corrected the canonical
  empty-target CAS and shipped in v1.31.2; post-tag test-only PTY cleanup PR
  #769 merged as `71653f00`. PR #770 corrected copied operator-pack run-start
  resolution and merged as `b836d064`; its independent code, platform,
  conformance, frontend, and browser gates were green, with an explicit owner
  override for the sole lingering full-preflight/Phase-231 path. Annotated
  v1.31.2 and v1.31.3 releases each published 13 assets and their public module
  provenance resolves the exact tagged commits. Downstream deployment and
  acceptance remain unclaimed. See
