...
|220 | `extra_instructions` on `RunOverrides` (D-365, upstream HA-45 narrowed) — the field already exists in the planner, already composes additively into `<additional_guidance>`, and already SURVIVES `SystemPromptOverride` (`buildSystemContent` receives the extra guidance on both branches of `baseRequest`); it is simply not on the Protocol surface, so a Protocol client cannot reach it. One optional wire field, zero new prompt semantics. **The precedence decision is a privilege-inversion argument, not a taste one:** `governance.set_tenant_overrides` is ADMIN-gated and `runs.set_overrides` is NOT, and today a session caller cannot remove the tenant's additive block at all — so per-field last-writer-wins would hand a non-admin caller a silent delete on an admin-set compliance block. The run-level value therefore COMPOSES BELOW the tenant value (tenant first, `\n\n` join, one production composition site) and there is no run-level clear; refusal was also rejected (an invisible admin-owned condition hard-blocking a per-run contribution reproduces the "who reconstructs the rest" problem). **Trust is stated rather than gated:** `<additional_guidance>` is joined raw with no escaping, so any caller of `runs.set_overrides` writes operator-trusted prompt text — but this is NO new authority class, because `system_prompt_override` sits on the same struct, same caller, and is strictly more powerful; gating the weaker capability while the stronger stays open would be incoherent, so the method's authorization tier is named as an OPEN RISK rather than silently resolved. Binding non-goal: this is NOT the home for recalled conversation memory (that is 219's untrusted-framed tiers) — a trusted position must not hold user-authored content | internal/protocol + internal/runtime/serve | §5.2, §6.5 | 219 | measured floors per package (see plan) | Shipped |
...
- **What it delivers:** D-364 — `StartRequest.caller_memory`, one additive optional `json.RawMessage` on `start`, composed into the UNTRUSTED-framed External memory tier under ONE fixed runtime-owned key (`caller_supplied`). The caller names no key, so runtime producers may add siblings forever and can never collide, and a caller can never shadow or displace a runtime key. `Conversation` stays runtime-only. Bounded at the edge by `maxCallerMemoryBytes` (32 KiB), refused `invalid_request` **before** `Spawn` so no session row and no task materialise, with the refusal text naming the field. Announced by the new canonical `memory.caller_block_admitted` event carrying `bytes` / `tier` / `key` and no fragment of the content, emitted at admission so it lands whether or not the run later succeeds. Additive only; `ProtocolVersion` holds at `0.1.0`. **This phase owns the generated wire manifest for the wave's Stage 1** (D-209 / D-223).
...
- **Decision:** D-364. **Status:** Shipped (v1.25).
...
- **What it delivers:** D-365 — REACH, not a new mechanism. `planner.LLMOverrides.ExtraInstructions` has been additive, verbatim-rendered and `SystemPromptOverride`-surviving since the tenant-override completion; it was simply absent from `RunOverrides`, so its only producer was the admin-gated tenant record. One optional wire field, zero new prompt semantics, no new section, no new error code, no `ProtocolVersion` move. **The run-level value COMPOSES BELOW the tenant value** — tenant first, joined by a blank line, in one production site — because `governance.set_tenant_overrides` is admin-gated and `runs.set_overrides` is not, so per-field last-writer-wins would hand a non-admin caller a silent delete on an admin-set compliance block. **There is no run-level clear**, and a present-but-empty value returns the tenant value's exact pointer untouched. **Binding non-goal:** `<additional_guidance>` is verbatim and unescaped, so it is NOT the home for recalled conversation memory or any user-authored text — that is 219's untrusted-framed tier, and the field's godoc names it, pinned by a smoke guard.
