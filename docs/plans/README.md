...
|159 | Serve-band promotion: the config→listener composition leaves `package main` (`harbor serve` serves the Protocol from stock yaml, but a scaffolded agent carrying compiled in-process Go tools cannot — `bootDevStack` + `devBootOptions` + the `devStack` serve/close lifecycle are trapped in `cmd/harbor`; promote that band into ONE importable internal package `internal/runtime/serve` — `internal/server` is already the protocol-server package, do NOT collide — leaving dev-only policy behind in `cmd/harbor`: the mock-LLM escape hatch (D-089), the hot-reload supervisor (D-099), the dev-token mint + bootstrap-token endpoint, draft scaffolding, and Console embedding (D-091); the promoted constructor carries NO `allowMock` knob and NO dev-signer; the auth-validator factory stays the single posture seam — non-nil = production posture (dev-only surfaces not mounted), nil = dev posture; `harbor serve`/`harbor dev`/`harbor console` become thin callers; SECOND CONSUMER same-wave (§13): `harbortest/devstack` re-wired onto the promoted band, deleting its hand-mirrored transports/mux block — the same move D-197 made for assembly; pure promotion — no new options surface, no wire changes, no new Protocol methods, no `ProtocolVersion` bump; D-025 served-handle concurrent-reuse N≥100 `-race` + goroutine-baseline teardown; D-291) | internal/runtime/serve + cmd/harbor + harbortest/devstack | §5.6, §5.4, §5.5, §6.1, §8 | 64, 110d, 118 | 85% | Pending |
|160 | `sdk/server` facade + `harbor scaffold --with-server` + parity gate (the promotion's EXTERNAL consumer: a curated `sdk/server` facade — `server.Open(ctx, cfg, Options{RegisterCatalog func(tools.ToolCatalog) error})` → a handle with `Serve`/`Close`, D-204 alias/forward over `internal/runtime/serve` NOT raw protocol re-exports — so a scaffolded agent with compiled in-process Go tools serves the Protocol at parity with stock `harbor serve`; PRODUCTION-ONLY posture, non-negotiable: `Open` ALWAYS builds the JWKS validator from `cfg.Identity` and fails loud (named field) when absent — NO dev-signer, NO mock knob; local-dev loop documented via `harbor token keygen` → `identity.jwks_file` → `harbor token mint`; `Open` re-runs `Validate` on programmatic configs so there is no validation bypass; `RegisterCatalog` fires at the existing pre-policy `assemble.Options.PreRegisterTools` seam — BEFORE builtin registration and the `tools.entries` Builder wrapping — so compiled tools receive declared approval/OAuth/policy wrapping, an adapter over the seam NEVER a second registration path (post-assembly `Catalog.Register` does NOT get wrapping — the named trap, §13); `harbor scaffold --with-server` (opt-in; default scaffold stays headless RunOnce) generates `cmd/<agent>/main.go` (loads yaml via `--config`/`--bind`, blank-imports `sdk/drivers/prod`, passes `agent.RegisterTools` to `server.Open`, serves); `harbor serve` calls the promoted constructor directly with a nil registrar (not via the facade); PARITY GATE (`test/integration`): boots `harbor serve` and a scaffolded `--with-server` binary from the SAME config and asserts (a) manifest-driven method-parity probe, (b) generated-tool discovery + dispatch through the catalog, (c) approval-gate wrap FIRES on both (proves pre-policy registration), (d) dev-only surfaces 404 on BOTH, (e) §17.3 real drivers + identity propagation + ≥1 failure mode + N≥10 stress + `-race`; NO new Protocol wire types — no D-223/D-209 churn; §18 same-PR: `scaffold-a-harbor-agent` + `add-an-in-process-tool` + `use-the-harbor-protocol` + `embed-harbor-headless` recipe companion + docs/site stubs/nav + README pointer; D-292) | sdk/server + cmd/harbor + test/integration | §3.6, §5.6, §5.5, §6.4, §8 | 159, 112a, 112b, 133, 131d, 118 | 85% | Pending |
...
- **What it delivers:** `harbor serve` serves the Protocol from stock yaml, but
  ...
  protocol-server package — do not collide). Dev-only policy STAYS in
  `cmd/harbor`: the mock-LLM escape hatch (D-089), the hot-reload supervisor
  (D-099), the dev-token mint + bootstrap-token endpoint, draft scaffolding,
  and Console embedding (D-091) — the promoted constructor carries NO
  `allowMock` knob and NO dev-signer. The auth-validator factory remains the
  single posture seam: non-nil = production posture (dev-only surfaces not
  mounted), nil = dev posture. `harbor serve` / `harbor dev` / `harbor console`
  become thin callers. The §13 same-wave second consumer is `harbortest/devstack`,
  re-wired onto the promoted band with its hand-mirrored transports/mux block
  deleted — the same re-homing move D-197 made for `assemble.Assemble`. Pure
  promotion: no new options surface, no wire changes, no new Protocol methods,
  no `ProtocolVersion` bump. The served `Handle` is a compiled artifact — a
  D-025 N≥100 concurrent-request `-race` test + goroutine-baseline teardown pin
  it. See `docs/plans/phase-159-serve-band-promotion.md`.
- **Decision:** D-291.
- **Status:** Pending.
...
- **Subsystem:** sdk/server (the curated facade), cmd/harbor (the
  `--with-server` scaffold flag + template), test/integration (the parity
  gate).
...
- **What it delivers:** a curated `sdk/server` facade over
  ...
  `RegisterCatalog` fires at the existing `assemble.Options.PreRegisterTools`
  seam — before builtin registration and the `tools.entries` Builder wrapping —
  so compiled tools receive declared approval/OAuth/policy wrapping; it is an
  adapter over that one seam, never a second registration path (post-assembly
  `Catalog.Register` does NOT get the wrapping — the named trap, §13). `harbor
  scaffold --with-server` (opt-in; default scaffold stays headless RunOnce)
  generates `cmd/<agent>/main.go` (loads yaml, `--config`/`--bind` trio
  mirroring `harbor serve`, blank-imports `sdk/drivers/prod`, passes
  ...
  directly. The wave's acceptance centerpiece is the parity gate
  (`test/integration`): boots `harbor serve` and a scaffolded `--with-server`
  binary from the SAME config and asserts (a) a manifest-driven method-parity
  probe, (b) generated-tool discovery + dispatch through the catalog, (c) an
  approval-gate wrap FIRING on the generated tool in BOTH binaries (proving
  pre-policy registration, D-292), (d) dev-only surfaces 404 on BOTH, (e) §17.3
  real drivers + identity propagation + ≥1 failure mode (401) + N≥10 stress +
  `-race`. No new Protocol wire types — no D-223/D-209 churn (stated
  explicitly). §18 same-PR surfaces: `scaffold-a-harbor-agent` +
  `add-an-in-process-tool` (+ `use-the-harbor-protocol` checked),
  `embed-harbor-headless` recipe companion, docs/site stubs + nav, README
  pointer. See `docs/plans/phase-160-sdk-server-scaffold-parity.md`.
- **Decision:** D-292.
- **Status:** Pending.
