...
|109j| Console pushes tool-input/tool-result into the app after `ui/initialize` (official AppBridge `sendToolInput`/`sendToolResult`) — the Data Delivery Console half | web/console | §6.4, §7 | 109i, 109b | n/a (Console) | Shipped (V1.1.x) |
|109k| MCP Apps spec-conformance hardening (wave-end audit fixes: `mimeTypes` UI capability not `displayModes`; server-namespaced app→host tool calls; size-changed + teardown + live-theme host obligations) | internal/tools/drivers/mcp + internal/mcpconsole + internal/protocol + web/console | §6.4, §7 | 109a, 109b, 109h, 109i, 109j | 80% | Pending |
