...
|176 | Session reopen: re-activate a closed session so a consumer chat resumes (AMENDS a Settled RFC decision — §6.9 "reopen-after-close is forbidden. Clients open a new session." becomes "a closed session — explicit OR GC-reaped — MAY be reopened"; the consumer-chat / white-label model needs conversations always-resumable, a user returning days later on the SAME conversation; reopen is CLEAN because close/GC reap the session RECORD not the DATA — `Registry.Close`/the GC sweep mark `Closed=true` + drop the live `openSessions` entry but leave the durable events/state/memory intact (the GC guards against resurrecting the record), the durable event log is gap-free + UNTRIMMED in V1 (no TTL/cap), the StateStore has no retention sweep; `Open`/`EnsureOpen`/`start` on a `Closed=true` record RE-ACTIVATES it — clears `Closed`/`ClosedAt`/`ClosedReason`, preserves the IMMUTABLE identity (a reopen under a DIFFERENT tenant is still `ErrSessionIDReuse`, invariant 3 unchanged), refreshes `LastSeen`, re-adds to `openSessions`+catalog, lifts the erasure fence, emits a NEW content-free `session.reopened` SafePayload — history resumes intact; THE ONE terminal exception = an ERASED session (Phase 130 `session.erase`): reopen fails loud `ErrReopenAfterErase` (NEW sentinel; never a silent empty-start — §5 fail-loud, §7 right-to-erasure), detected against the erasure's observability-scope records — a pending erasure LEDGER (in-flight/interrupted) OR a NEW durable content-free erasure TOMBSTONE (`erasureTombstoneKindPrefix`, written in `completeErasure`'s success criteria + RETAINED — sibling of the `session.erased` record-of-fact, same content-free shape so no new retained info; REQUIRED because the pending ledger is deleted on erasure success so it alone can't make a converged erasure terminal); race-safe erased-check — reopen's load→check→reactivate→save + the tombstone write both under one `r.mu` critical section (mirrors catalog erasure-vs-open serialization, `catalog.go:82-149`) so reopen sees pre-erasure OR post-erasure, never mid-deletion resurrection; ALL §6 isolation preserved — identity-mandatory + immutable (`ErrIdentityMismatch` on stored-vs-ctx mismatch), no identity-downgrade knob; old blanket `ErrReopenAfterClose` retired from the reopen path (`Touch`-on-closed keeps a loud read-only guard renamed `ErrSessionClosed`); `protocol.ErrSessionReopenAfterErase` → `CodeInvalidRequest` (same code the retired reopen-after-close used — NO new wire error CODE, `ProtocolVersion` unbumped); §13 same-wave consumer — Playground/Sessions resume a closed session (a `start` on a closed id now succeeds) + refresh on `session.reopened`; ONE new canonical event = full D-223 lockstep + D-209 events.md regen; §17.1 integration test (real durable+inmem state/events/memory + real `CascadeEraser`: close→reopen→history intact on DURABLE; erase→reopen fails loud; cross-tenant + identity-mismatch reject; ≥1 failure mode; `-race`) + registry D-025 stress extended N≥100 incl. the reopen-vs-erase race; D-312) | internal/sessions + internal/protocol + internal/runtime/serve + web/console | §6.9, §5.2, §6.13, §7 | 130, 155, 125, 73c, 106, 118 | 85% | Pending |
...
- **What it delivers:** AMENDS a Settled RFC decision. RFC §6.9 was
  ...
  `ClosedReason`, preserves the immutable identity (a reopen under a DIFFERENT
  tenant is still `ErrSessionIDReuse`, invariant 3 unchanged), refreshes
  `LastSeen`, re-adds to `openSessions`+catalog, lifts the erasure fence, emits a
  new content-free `session.reopened` SafePayload — history resumes intact. THE
  ONE terminal exception: an ERASED session (`session.erase`) fails loud
  `ErrReopenAfterErase` (new sentinel; never a silent empty-start — §5, §7),
  detected against a pending erasure ledger (in-flight) OR a new durable
  ...
  success so it alone can't make a converged erasure terminal). Race-safe: reopen
  and the tombstone write serialize on one `r.mu` critical section (mirrors
  `catalog.go:82-149`) so reopen sees pre- OR post-erasure, never mid-deletion
  resurrection. All §6 isolation preserved (identity-mandatory + immutable,
  `ErrIdentityMismatch`; no downgrade knob). No new wire error CODE
  (reopen-after-erase reuses `CodeInvalidRequest`); one new canonical event =
  full D-223 lockstep + D-209 events.md regen; `ProtocolVersion` unbumped. §13
  same-wave consumer: Playground/Sessions resume a closed session + refresh on
  `session.reopened`. §17.1 integration test (real durable+inmem drivers +
