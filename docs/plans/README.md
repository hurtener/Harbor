...
| 84b| Bifrost extended multimodal (full SDK surface + streaming) | llm/drivers/bifrost + planner | §6.5, §11Q3 | 84a, 28, 32, 49, F11/D-166 | 88% | Pending (V1.1.x) |
...
| 85g| MCP Apps host (Console `ui://` renderer)      | web/console          | §6.4, §7    | 28, 85a               | 80%  | Revisit after RC-final (verify) |
...
|107d| Native parallel tool-calls (dev executor `CallParallel` branch + React `CallParallel` emission + default flip; closes 107c's serialization carve-out) | cmd/harbor + internal/runtime/parallel + internal/planner/react + internal/config | §6.2, §6.5 | 107c, 47, 83i | n/a | Pending (V1.1.x) |
...
- **85g — MCP Apps host.** Console-side `ui://` renderer; sandboxed iframe; strict CSP; AppBridge `postMessage` JSON-RPC dialect. Closes the `registry.go` primitive-without-consumer gap. RC blog does not explicitly carry Apps in its deprecation list, but Apps was experimental in 2025-11-25 — the RC's extension-stabilization policy may reshape `_meta.ui.resourceUri` or move Apps into a versioned extension. RFC §6.4, §7. Deps: 28, 85a. **Revisit after RC-final** (2026-07-28) — verify Apps' position in the RC; if intact, implement; SDK-independent (Console-side TS work). See `docs/plans/phase-85g-mcp-apps-host.md`.
