...
|265 | User-scoped signed OAuth MCP capability lifecycle (D-448): closed-envelope/descriptor sibling methods, verified `agent_config:user` plus signed reach, tenant/user/session derived from bearer, `ConfigScopeUser` desired pairs, full physical owner and exact teardown, acting-user projection narrowing, private initialize/discovery bearer, two-user isolation, and generated Protocol/TypeScript/docs/smoke surfaces | agent-config signed OAuth MCP + physical MCP registry/provider + projection + Protocol | §5.5, §6.4, §6.11, §6.16 | 130, 205, 206, 211, 246, D-397, D-398, D-401, D-407 | focused package/race + generators + two-user isolation + static smoke | Accepted for next Harbor patch release; downstream/runtime deployment and acceptance pending |
