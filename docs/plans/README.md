...
- **Deps:** 28 (MCP southbound driver), 30 (tools/auth — the RFC 8414
  ...
  Related, NOT a dep: the parked 92p (reserved D-246) — the
  runtime-side-flow sibling this reconciles with.
- **What it delivers:** near-zero-config credential onboarding as DATA. When
  ...
  discovered requirement marked unverified. One discovery mechanism, two
  consumption postures: the parked 92p reuses this chain if unparked
  (pointer note added to its plan). Full D-223/D-209 regen; no new method
  or event. See `docs/plans/phase-164-mcp-oauth-discovery-surfacing.md`.
- **Decision:** D-297.
- **Status:** Pending.
