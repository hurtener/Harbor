...
- **Evidence:** Focused in-memory/SQLite/Postgres conformance, cancellation,
  ...
  direct-5432 acceptance are required. The hosted `state-postgres` job must
  select the exact real-Postgres repair test and assert its exact PASS line;
  skips, renamed selectors, and no-test matches are failures. The phase smoke
  statically guards the plan, register, recipe, config/scaffold guidance, and
  hosted selector without contacting a database. Local preflight is not run.
- **Decision:** D-433. **Status:** In implementation for v1.29.3; extends
  HA-69 without allocating a new ask identifier.
