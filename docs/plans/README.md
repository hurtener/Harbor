...
|212 | Artifact read-path byte correctness + loud reference-resolution failure (D-357): two defects that each turn a recoverable situation into a silent-or-fatal one. `artifact_fetch` builds its response as `Content: string(window)` over a `Content string` field, so any byte sequence that is not valid UTF-8 is silently replaced with `U+FFFD` — a PDF, an image or a zip comes back CORRUPTED with no error and no signal, while its Protocol twin `types.ArtifactsGetResponse.Content` is already `[]byte` and already correct. The fix is NOT base64-into-context (a 1 MiB PDF is ~350–470k tokens of base64 the model can do nothing with) but a loud refusal naming the MIME and the failing offset and POINTING AT the by-reference route — the §13 fail-loud posture, where the current behaviour is the silent-degradation shape §13 names. The admissibility check cannot be MIME-driven alone (heavy tool results are stamped `application/json` unconditionally) and cannot be naive UTF-8 validation either (a window boundary splits multi-byte runes), so the window is TRIMMED to whole runes at both ends and `returned_bytes` reports the trimmed length. Second half: an artifact-reference parameter that fails to resolve currently terminates the dispatch step (`callTool` returns the invoke error up), while the sibling `callParallel` path explicitly treats a branch's resolve/validate failure as THAT BRANCH'S RESULT, not a whole-call abort — an asymmetry between siblings, not a missing feature. A resolution failure becomes a tool-result observation naming the unresolvable id so the model can correct on its next turn; `ErrNoResolver` stays step-terminating because a missing resolver is a runtime misconfiguration the model has no recovery for. The failure observation deliberately does NOT restate the session's artifact list — the `<session_artifacts>` block already carries it every turn | internal/tools/builtin + internal/tools/artifactref + internal/runtime/dispatch | §6.4, §6.5, §6.10 | 210, 209 | 85% | Pending |
|213 | Heavy-content threshold rebalance + search-preview split (D-358): `DefaultHeavyOutputThresholdBytes` is 32 KiB and is single-sourced into FOUR consumers with materially different jobs — the offload threshold, the LLM-edge `findContextLeak` guard, the `internal/runtime/dispatch` safety floor, and `search.HeavyPreviewThreshold`. At 32 KiB (~8k tokens, ~4% of a 200k window) an ordinary 40 KiB JSON tool result is promoted to a stub, so the model spends a SECOND tool call reading back what it just produced: one call becomes three. The threshold moves to 128 KiB (~32k tokens) and the trajectory payload budget follows automatically (`threshold - trajectoryPayloadHeadroom`). **The leak-guard objection is recorded and answered rather than left to be re-derived:** `materializeRequest` runs FIRST at the same threshold and offloads, and `findContextLeak` catches what materialization could not — they are a designed pair, so moving them together preserves the relationship; the guard's job is "something got past the offload", not "nothing is ever this large". **One consumer is split off because 128 KiB is wrong for it:** `search.HeavyPreviewThreshold` bounds a result-row PREVIEW, and a list of ten previews at the offload threshold would exceed most context windows on its own — it gains its own 32 KiB constant with a godoc stating why a preview bound is not an offload bound. That is a documented SPLIT of the single-sourced value, not a forbidden duplicate copy, and the single-sourcing comment is amended to say so. The `artifact_fetch` ceilings (`fetch_default_max_bytes` 64 KiB / `fetch_hard_max_bytes` 1 MiB) are deliberately UNTOUCHED — they bound a deliberate read-back, not an automatic promotion. An operator's explicit `heavy_output_threshold_bytes` still wins (§10) | internal/config + internal/search + internal/llm + internal/runtime/dispatch | §6.5, §6.10, §6.13 | 210, 209 | 85% | Pending |
|214 | The MCP arm of pass-by-reference routing (D-359): phase 210 shipped the in-process arm and deferred the third-party arm on three named blockers, ALL of which concern handing a remote consumer something it can DEREFERENCE. This phase delivers a different shape those blockers do not reach — the runtime resolves the reference itself and places the BYTES into the outbound MCP tool call. **A DOCUMENTED DEPARTURE from D-347 part 5, recorded rather than applied quietly:** the ADDRESS blocker dissolves (nothing dials in; the runtime dials out on a connection the operator configured), the GRANT'S-SEMANTICS blocker dissolves (there is no grant, so no retry / redirect / HEAD-then-GET / partial-transfer semantics to define), and the CREDENTIAL OBLIGATION transforms into "the outbound body must not be persisted unredacted", which §7 rule 7 and §13 already impose. D-347's deferral stands unchanged for the URL-minting design it actually describes. **Two genuinely hard parts, both named:** (1) the carrier's projection bound — today the invariant holds BY CONSTRUCTION because `Ref.MarshalJSON` / `String` / `LogValue` all emit the id and there is no way to marshal content; this phase creates one fenced egress encoder, so the property becomes "by construction except at one reviewed site", held there by extending `ScanSubstitutionSites`' AST allow-list; (2) an MCP tool's `inputSchema` is authored by the REMOTE SERVER, so there is no Go type to reflect over and `TypeContainsRef`/`Substitute` do not transfer — the mapping is OPERATOR-declared per server/tool, writable over the Protocol and persisted in the revision like its sibling connection fields (a SERVER-declared annotation is considered and REJECTED: it would let a remote server decide when the runtime reads its own store). **It deliberately does NOT take the D-338/D-340/D-346 fail-closed boot gate, and the reasoning is recorded because the repo's pattern-match would add it by reflex:** those gates exist because the field determines WHERE A CREDENTIAL IS SENT (D-300); this field names a parameter, carries no secret, and grants no reach the connection did not already have — the resolver stays run-scoped, so the only movable bytes are ones the run's own identity already reaches, to a server the tenant's admin already attached. Containment is a per-connection byte-ELIGIBILITY flag. Egress gets its OWN ceiling, MiB-scale, NOT derived from the heavy threshold (substituted bytes never enter a model's context, so the budget is network+memory, not tokens), and an oversize value FAILS LOUD rather than truncating — a partial document delivered to an ingester is a corruption, not a bounded read, so the read path's truthful-truncation posture deliberately does not apply. The accepted residual risk is stated as a property, not an open question: an agent that can read artifact X and call server Y can move X to Y | internal/tools/artifactref + internal/tools/drivers/mcp + internal/runtime/agentcfg + internal/protocol + internal/config | §6.4, §6.5, §6.10 | 212, 210, 209, 206 | 85% | Pending |
|215 | Caller-named agent selection (D-360, HA-44): `agent_id` is an explicit Protocol argument on ~30 request types — the whole `agent_config.*` family, the user-layer and skills families, `tools.describe` — and absent from exactly one place: the surface that STARTS a run. `StartRequest` (`types/control.go:94`) has no agent field, `tasks.SpawnRequest` has none, and the run loop reads its agent from `opts.AgentConfigID`, a CONSTRUCTION-TIME option seeded from the constant `devAgentConfigID = "harbor-dev-agent"` (`serve.go:416`). **The consequence is concrete rather than forward-looking: a caller can create agent configs that can NEVER RUN** — write a revision under a new agentID, `agent_config.get` reads it back, `diff` and `rollback` work, and no run will ever use it. **An earlier sizing that read the projections as per-runtime is corrected in the plan:** every projection takes `agentID` as an ordinary argument (`ActiveLLMOverrides(ctx, reg, agentID, id)` and nine siblings), so that layer is a parameter swap; and the genuinely stateful reconcile legs are ALREADY partitioned by `auth.Owner{Tenant, Agent}` (`projection.go:186`, phase 206), so the multi-agent thrash hazard is already closed. Work concentrates at the edge (validation) and the boundary (carry-through). Omitted ⇒ byte-identical to today; present-but-unresolvable ⇒ REFUSED at the Protocol edge before the task exists, never a substituted default (a caller that named A, got B, and was told it succeeded reproduces the defect one layer down); persisted onto the task and the session row so D-309's representable-absence stops applying to runs that DID name an agent. Validation reuses the tenant-scoped registry lookup the `agent_config.*` family already performs — NOT a new entitlement plane — and the refusal must be indistinguishable from not-found or it becomes a cross-tenant existence oracle. **The four-tier prompt composition already ships and is unchanged** (admin Base > admin User > durable User > session User): replacement-vs-layering is already expressible as Base-set vs User-set so NO `mode` discriminator is added (§4.4), and a user layering onto an admin-authored agent needs NO chaining because both tiers key on the same agentID under different `ConfigScope`s. Publication/visibility is explicitly OUT of scope — the consumer Console owns which agents it offers | internal/protocol + internal/tasks + internal/runtime/serve + internal/sessions | §5.2, §6.2, §6.16 | 206, 211 | 85% | Pending |
|216 | User-scope MCP connections (D-361): the `ConfigScopeUser` tier already composes into a run for prompts, skills membership and tool exposure, but NOT for connections — all three reconcile legs read `ConfigScopeAgent` exclusively (`projection.go:171`, `:249`, `:324`) and `AgentConfigUserPayload` carries no connections field. So a user can curate WHICH of the admin's tools they see and extend the prompt, but cannot bring their own MCP server. **The credential plane is held out of scope BY CONSTRUCTION, not by policy:** the user-scope descriptor declares NO credential field — no `injection`, no `oauth_provider`, no inline `oauth`, no static `Authorization`, no attach-time `Headers` — mirroring how `AgentConfigUserPayload` declines to declare `Base`, so a `DisallowUnknownFields` decode rejects one by name and there is no validator to bypass at a second door. Stdio is refused outright (a user-declared subprocess is an RCE surface; the fail-closed allowlist is an operator control). D-300's invariant would be reopened from the LEAST-privileged direction by a later "convenience" field, so D-361 states it as an invariant rather than a current implementation detail. **The bare-name collision question is the phase's central design work and is deliberately left OPEN in the plan rather than pre-decided:** the process-global bare-name model is load-bearing (D-287/D-301, preserved through 206 and 211), so resolution belongs at the PROJECTION layer, and the plausible answers (per-user namespacing / first-writer-wins with a loud refusal / explicit admin-wins precedence) have materially different operator-facing behaviour — the author picks one, justifies it against D-287, and records it; picking silently is the named failure mode. The `auth.Owner{Tenant, Agent}` type needs a user axis or a defined reason it does not, touching every owner-scoped call site from 206 and 211 — sized as the largest mechanical piece. Per-user OAuth for user-declared servers is named as the SUCCESSOR question, not answered | internal/runtime/agentcfg + internal/agentcfg + internal/protocol + internal/tools/drivers/mcp + internal/tools/auth | §6.4, §6.16, §7 | 215, 206, 211 | 85% | Pending |
|217 | `meta_annotations` honour `_meta` path nesting (D-362, HA-43): two mechanisms write into the SAME outbound MCP `_meta` map on the same call and disagree about what a dotted key means. `injection.meta_key` is a dot-separated PATH — `injectMeta` walks it creating intermediate maps, so `vendor.apiKey` lands at `_meta.vendor.apiKey`. `meta_annotations` merge FLAT — `buildIdentityMeta` (`mcp.go:1391-1396`) does `meta[k] = v` verbatim with the reserved check on the WHOLE key, so `vendor.accountId` lands as the single literal key `_meta["vendor.accountId"]`. A receiver-style server reading one nested namespace can therefore be handed a credential but NOT its non-secret companion value, by any route (it cannot ride `injection` — one mapping per connection, already spent, and `accountId` is correctly refused by the credential-name predicate; it cannot ride `Headers` — documented secret, never persisted). The annotation merge honours the same nesting using the SAME helper and the SAME guard (§13 — adapt the call site, never fork the helper), the reserved check becomes PER-SEGMENT (strictly tighter — `tenant.foo` is refused where today it is admitted), and `maxInjectionMetaKeyDepth` extends to the annotation path since only injection could previously nest. **Explicitly NOT widening `receiverInjectionCredentialSegments`** — it is a security control guaranteeing any declarable target key is a key the redactor holds to `***`, and the companion value is not a secret and does not want the secret channel. **The §10 backward-compatibility question is an acceptance criterion, not a note:** `validate.go:1780-1787` today rejects only an empty key and a whole-key reserved match, so a dotted annotation key is LEGAL today and lands flat — after this phase it nests (a silent semantic change) and `tenant.foo` becomes a boot failure. Blast radius is plausibly zero (no in-tree caller populates the field) but §10 requires that be ESTABLISHED by survey and recorded in D-362, with a migration path if any populating caller exists. The named concurrency hazard is specific: a nesting helper that walks and creates intermediate maps can share or mutate structure across calls if handed the config's own map rather than building fresh | internal/tools/drivers/mcp + internal/config + internal/runtime/agentcfg | §6.4, §7 | 206, 211 | 85% | Pending |
...
- **Decision:** D-344. **Status:** Pending (v1.19).
