...
- **What it delivers:** closes the operator-confirmed live-test regression
  ...
  seam per producer on the one bus: the cost emit promotes to the LLM-edge
  wrapper chain (every driver; bifrost's internal emit deleted — one emit per
  call, never two) and the tool lifecycle emits move to the
  transport-agnostic dispatch seam with the full run quadruple on the
  envelope (inproc's per-driver emits deleted) — which also fixes the latent
  live attribution bug in the same PR (§17.6). Payloads stay CONTENT-FREE
