...
|177 | Projection-completeness gate + the populate/remove surfaces (HA-24; v1.14; the CLASS that HA-22/174 is one instance of — declared-but-never-assigned wire fields with filters/aggregates over them (false absence), on THREE more surfaces + a mechanical class-closer. TASKS: `TaskRow.HasPendingApproval` is READ by the `tasks.list` filter (`list.go`) but NEVER assigned by the sole producer `projectRow` (`registry_projector.go`) → `has_pending_approval:true` returns an EMPTY page on a fleet with open gates (the sharp one) → populate at projection time from the approval/pause registry; `BackgroundAcknowledged` (no filter) → represent/populate; ALSO the WIRED tasks `serve.Enricher` (`enricher.go`) is a STUB — `ParentSession` returns a zero `TaskParentSessionRef{}`, `Cost` a zero `TaskCostRollup` — so even the surface Harbor got STRUCTURALLY right ships zeros → un-stub the parent-session card (session registry) + cost rollup (cost events, coordinated with 174). FLOWS: `budgetConsumption` (`catalog.go`) sets `RequestsUsed`+`CostUSDUsed` but NEVER `TokensUsed` (non-omitempty) → fabricated 0 on `FlowSummary`/`FlowDetail` → add `RunRecord.Tokens` (symmetric with `CostUSD`) + sum it. MEMORY: producer never sets `AgentID`/`ExpiresAt`; V1 memory has NO TTL (`ExpiresAt` zero=no TTL) → `filter.has_ttl_expiring` + the `expiring_in_1h` aggregate (list+health) are STRUCTURALLY DEAD → REMOVE them (wire-shape change, D-223/D-209); `filter.agent_ids` → populate `AgentID` from the turn producer identity OR loud-reject over the unpopulated field (D-311), never a false-empty page. TOOLS: split to 178 — no production `Annotator` exists (only a `fakeAnnotator` test double, §17.8), so Phase 177 honestly GATES the annotator-backed facets/search/aggregates behind an annotator-wired capability (loud-reject/honest-partial when unwired) pending 178. THE GATE (centerpiece, the class-closer): a registry-gated projection-completeness check (`internal/protocol/projectioncheck`) — every projection surface self-registers a `ProjectionContract` (a probe that runs the PRODUCTION projector over a fully-populated record + the filtered/sorted/aggregated field-set + a reason-carrying honest-omission allow-list) and a build-time conformance test FAILS when a filtered/sorted/aggregated field is left zero and not allow-listed; a coverage half asserts every known surface is registered (a surface can't dodge the gate) — mirroring the events `RegisteredDrivers()` conformance gate (D-305). Honest exclusions NOT touched: `FlowBudget.TokenCap` (omitempty, zero=no cap), `MemoryItem.AgentID`/`.ExpiresAt` ROW fields (omitempty), `TaskParentSessionRef.SessionID` (populated). The gate is the primitive; the 3-surface fixes + 174's session fix + the tools interim-gating are its consumers so it lands green (§13). No `ProtocolVersion` bump. D-313) | internal/protocol/projectioncheck + internal/tasks/protocol + internal/runtime/serve + internal/runtime/flow + internal/memory/protocol + internal/protocol/types + web/console | §5.2, §6.1, §6.4, §6.6, §6.8, §7 | 174, 08, 54, 60, 73c, 107a | 90% | Pending |
...
- **Subsystem:** internal/protocol/projectioncheck (the new gate + registry),
  ...
  `agent_ids`), internal/protocol/types (memory wire removal), internal/tools/protocol
  (interim honest-gating), web/console.
...
- **What it delivers:** HA-24 — the CLASS that HA-22/174 is one instance of, on
  ...
  NO TTL, so `has_ttl_expiring` + the `expiring_in_1h` aggregate are structurally
  dead — REMOVED (wire-shape change, D-223/D-209); `agent_ids` populated from the
  turn producer identity or loud-rejected over the unpopulated field (D-311).
  TOOLS: split to 178 — no production `Annotator` exists, so this phase honestly
  gates the annotator-backed facets/search/aggregates behind an annotator-wired
  capability (loud-reject/honest-partial when unwired) pending 178. THE GATE (the
  class-closer): a registry-gated projection-completeness check — every surface
  self-registers a `ProjectionContract` (a probe running the production
  projector, the filtered/sorted/aggregated field-set, and a reason-carrying
  honest-omission allow-list); a build-time conformance test FAILS when a
  filtered/sorted/aggregated field is left zero and not allow-listed, and a
  coverage half asserts every known surface is registered — mirroring the events
  `RegisteredDrivers()` conformance gate (D-305). The gate is the primitive; the
  surface fixes + 174's session fix + the tools interim-gating are its consumers
  so it lands green (§13). Honest exclusions (`FlowBudget.TokenCap`, the
  `MemoryItem.AgentID`/`.ExpiresAt` ROW fields, `TaskParentSessionRef.SessionID`)
  are verified and excluded. No `ProtocolVersion` bump. See
