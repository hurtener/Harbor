...
below is **rejected on sight**. The kanban board is this page's primary view in the §5 depth bar (in place of a flat `DataTable`); it still mounts inside the shared shell, and the bulk-action toolbar is the shared `BulkActionBar` — not a forked per-page toolbar.

The page MUST:

- **Route under `(console)/`.** The page lives at
  `web/console/src/routes/(console)/tasks/` and is served at `/tasks` with
  **no `/console/` URL prefix** (the `(console)` route group is a
  ...
  `(console)/tasks/[id]/` and are served at `/tasks/<id>`. All inter-page
...
- **Use the shared `components/ui/` inventory.** It composes the cross-page
  ...
  page-specific components go in `components/tasks/`.
