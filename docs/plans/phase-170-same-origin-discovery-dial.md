...
pinned to, rather than applying one global refusal to both hops.

1. **Per-fetch dial pin carried on the fetch context.** `fetchHop` already
   computes the step, the target, and `serverOrigin`, and `validateHop`
   already computes `sameOrigin`. `fetchHop` attaches a `dialPin` value to the
   per-fetch `ctx` describing THIS hop:
   - `step` (`StepProtectedResource` | `StepAuthorizationServer`);
   - `sameOrigin` (as `validateHop` computes it);
   - `pinnedHostPort` — the operator-declared `ServerURL` host:port (from
     `DiscoveryInput.ServerURL`, a trusted boot-config origin — NOT the
     attacker-influenceable `ResourceMetadataURL`);
   - `pinnedIPs` — the resolved IP set for the pinned host, captured ONCE at
     the start of the walk (see the DNS-rebinding subtlety below).
...
2. **Dialer reads the pin post-resolution via `ControlContext`.** Replace
   ...
   resolved `ip:port` — the load-bearing property the current `Control` relies
   on) AND receives the `ctx` carrying the pin. The closure enforces:
   ...
   - Resolved IP is private AND the pin is the same-origin protected-resource
     hop AND the dial's host:port equals `pinnedHostPort` AND the resolved IP
     is a member of `pinnedIPs` → return nil (the relaxation).
...
3. **`validateHop` unchanged in intent; the fix is the dialer.** The two guards
   now agree: what `validateHop` permits at the origin-string layer, the
   dialer permits at the resolved-IP layer — but ONLY for the pinned target.
...
The relaxation therefore pins to the RESOLVED IP the connection legitimately
uses. The discoverer resolves `pinnedHostPort`'s host ONCE at the start of the
walk into `pinnedIPs`, carries that set on the fetch ctx, and the
`ControlContext` closure permits a private resolved IP for the same-origin hop
ONLY when it is a member of `pinnedIPs`. A rebind — or a same-origin-by-string
redirect — that resolves to a DIFFERENT private IP is refused because that IP
is not in the pinned set. This is provable in a unit test with two names that
resolve to two different loopback/private addresses.
...
- [ ] The same-origin RFC 9728 protected-resource-metadata hop to the
  ...
  private/loopback address — pinned to `pinnedHostPort` + `pinnedIPs`.
...
- [ ] **DNS-rebinding crux:** a same-origin-by-string metadata URL / redirect
  ...
  (proven to FAIL — i.e. wrongly permit — under a host:port-only pin with no
  resolved-IP membership check; the test is the discriminator).
- [ ] A redirect that leaves the pinned origin RE-VALIDATES and is refused
  exactly as today (bounded-redirect + re-validation preserved). Named test:
  `TestDiscoverer_RedirectOffPinnedOrigin_StillRefused`.
...
- `internal/tools/auth/discovery.go` — `NewDiscoverer` swaps
  `Control` → `ControlContext`; the closure reads the per-fetch `dialPin` from
  ctx and applies the same-origin-pinned relaxation; `fetchHop` resolves the
  pinned host once per walk into `pinnedIPs`, builds the `dialPin`, and threads
  it onto the fetch ctx; a small `dialPin` ctx-key type + resolver helper.
  `validateHop` unchanged (its same-origin computation is reused to set the
  pin's `sameOrigin`). `WithPrivateNetworkAccessForTest` / `allowPrivate`
  unchanged (test-only).
- `internal/tools/auth/discovery_test.go` — migrate the protected-resource
  ...
  add the crux rebind test, the cross-origin-still-refused test, the
  redirect-off-origin test, the AS-still-needs-allowance test, the concurrent
  pin-isolation test; retain the existing guardrail negatives. Rework
  `TestDiscoverer_Discover_DialTimeHostnameToPrivateRefusal` (`:400`) — which
  today ASSERTS the bug (a same-origin hostname→loopback hop is refused) — into
  the pinned-relaxation positive plus the rebind negative.
...
- **Unit:** the five named security tests above (production-path completion,
  cross-origin still-refused, rebind-to-different-private-IP crux,
  redirect-off-origin, AS-still-needs-allowance); the retained guardrail
  ...
  (public IP allowed; same-origin private in-pinned-set allowed; same-origin
  private out-of-pinned-set refused; cross-origin private refused; missing pin
  → fail-closed refuse).
- **Integration (`test/integration/phase170_mcp_oauth_discovery_dial_test.go`):**
  ...
  RFC 9728 §3.2 + RFC 8414 §3.2 example documents) bound to loopback and
  reached via a same-origin hostname. Production-path discoverer (NO test-only
  escape) → protected-resource hop COMPLETES → AS hop surfaces
  `needs_allowance` (no allowance) — the self-hosted posture proven
  end-to-end. Plus: the rebind negative (a second name resolving to a different
  private IP refused), the cross-origin negative, identity propagation on the
  probe/read path + a cross-tenant read refusal (≥1 failure mode), `-race`. An
  env-gated live leg (`HARBOR_LIVE_MCP_OAUTH`) against a real
...
- Class `unit-tests` (the dial policy needs the spec-derived fixture server,
  ...
  `test/integration` phase-170 test; plus a static grep asserting no production
  code path sets `allowPrivate` outside `WithPrivateNetworkAccessForTest`
  (the "no production private-network knob" invariant, mechanically guarded).
