...
**SvelteKit scaffold ownership (audit-resolved A5):** Phase 72h is the
phase that **introduces `web/console/`**. Beyond this phase's TypeScript
modules (lib/db/), 72h MUST also land the SvelteKit scaffold
infrastructure so every downstream Stage-2 page phase can rely on it
being in place without serial deps among parallel Stage-2 phases.
Specifically 72h ships:

- `web/console/package.json` — `npm`-managed; pins `svelte: ^5.0.0`
  (D-092), `@sveltejs/kit`, `@sveltejs/adapter-static` (D-091 — runes-
  mode SPA), `@skeletonlabs/skeleton` (CLAUDE.md §4.5 rule 4),
  `typescript`, `vitest`. The dependency set is the union of what 72h's
  lib/db/ needs (TypeScript + Vitest) AND what every downstream
  Stage-2 page needs (SvelteKit + Svelte 5 + Skeleton + Stylelint).
- `web/console/tsconfig.json` — strict TypeScript config + path aliases
  (`$lib` → `src/lib`).
- `web/console/svelte.config.js` — `compilerOptions: { runes: true }`
  per D-092; adapter-static per D-091.
- `web/console/vite.config.ts` — SvelteKit plugin + Vitest test runner
  config (jsdom env, the `fake-indexeddb` polyfill 72h's tests need).
- `web/console/src/lib/tokens.css` — the design-token surface CSS
  custom properties (colors, spacings, type scale, radii, motion) per
  CLAUDE.md §4.5 rule 3. The file is empty-shaped at this phase — a
  later Console phase (or this phase as part of the integration test
  fixture) populates it; the existence of the file + its centralised
  location is what 72h's scaffold contributes.
- `web/console/.stylelintrc.cjs` — disallows hex / rgb() / named
  colors and arbitrary px / rem / em outside `tokens.css` per CLAUDE.md
  §4.5 rule 3. The `npm run lint` script invokes Stylelint + ESLint +
  svelte-check.
- `web/console/src/lib/protocol.ts` — generated stub committed by 72h
  with the `// CODE GENERATED BY cmd/harbor-gen-protocol-ts. DO NOT
  EDIT.` header (D-093). Downstream phases regenerate it as wire types
  are added; 72h ships the initial empty-but-typed-shaped stub +
  ensures `make protocol-ts-gen` produces a deterministic file.
- `web/console/src/routes/+layout.svelte` — minimal app shell (no UI
  beyond a `<slot />`) so every downstream Stage-2 phase can add its
  page route under `src/routes/`.

72h is still purely module-level for its OWN deliverables (no
Stage-2-page routes, no Skeleton components used by 72h itself — only
TypeScript + Vitest tests against lib/db/). The SvelteKit scaffold
above is **infrastructure** required by downstream Stage-2 phases; it
lands here so every Stage-2 page can ride on it in parallel.
