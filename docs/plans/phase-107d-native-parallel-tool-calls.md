...
- [ ] **AC-18** `docs/decisions.md` D-169 entry — pins the native-parallel cutover: the executor `CallParallel` branch, the `JoinKind`-collapses-to-`JoinAll`-on-native semantic, the non-atomic execution mode, the default flip, and the `parallel.Executor`-reuse (no second dispatcher). References D-167, D-056, brief 15 §6.
...
- `internal/planner/react/projector.go` — AC-8: native `CallParallel` emission gated on the knob; 107c serialization preserved on the OFF path. ~40 LOC.
...
- `internal/planner/react/projector_test.go` — AC-12 (native vs serialization emission).
