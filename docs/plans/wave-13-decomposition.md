...
| **72d** **(Shipped — D-109)** | `notification.*` event topic per Brief 11 §CC-3 (NEW event family; rules-engine-lite mapper from event taxonomy to notification class) | First page consumer: Overview page alert ribbon (Stage 2: 73a). Settings notifications-routing matrix (Stage 2: 73m). Stage-1 binding test consumer at `internal/runtime/notifications/subscriber_test.go::TestSubscriber_TaskFailedSynthesisesNotificationTaskFailed` satisfies §13 primitive-with-consumer compliance per §12 item 5. | 05, 06 |
...
| **72f** **(Shipped — D-111)** | Runtime posture surface: `runtime.info` / `runtime.health` / `runtime.counters` / `runtime.drivers` / `metrics.snapshot` | First page consumer: Overview counter cards + Settings Runtime Info card (Stages 2: 73a, 73m). Stage-1 §13 consumer: `test/integration/runtime_posture_test.go` exercises all five methods end-to-end. | 60 |
| **72g** **(Shipped — D-112)** | `governance.posture` (read-only `IdentityTiers` view per D-081) + `llm.posture` (provider/model/region/mock-flag per D-089) — extends the Phase 72f `PostureSurface` (one surface, not two) | First page consumer: Settings Governance Posture + LLM-Provider Posture cards (Stage 2: 73m). Stage-1 §13 primitive-with-consumer compliance is satisfied by the integration consumer `test/integration/phase72g_posture_test.go` (real governance + llm + transport + ES256 auth). | 36a, 36b, 64 (D-089), 72f |
| **72h** **(Shipped — D-113)** | Console DB local schema (per D-061): `saved_filters`, `saved_views`, `profiles`, `runtime_registry`, `auth_profiles`, `pat_store`, `notifications_routing`, `keybindings` — plus the `web/console/` SvelteKit scaffold (audit-resolved A5) | First UI consumer: Phase 73f Tools saved-filter chips (Stage 2); the in-package `tests/integration.spec.ts` is the §13 Stage-1 consumer per §12 item 6. | 60 (Protocol auth for PAT) |
...
| **73f** **(Shipped — D-116)** | Tools | `tools.list` + `tools.get` + `tools.describe` + `tools.metrics` + `tools.content_stats`; admin methods `tools.set_approval_policy`, `tools.revoke_oauth` (gated by the verified `auth.ScopeAdmin` claim — D-079 closed the scope set; there is NO `tools.admin` scope) | 72a, 75 |
...
| **73h** **(Shipped — D-128)** | Background Jobs | `tasks.list` filter `kinds=["background"]` (the plural `[]TaskKind` slice — A2 audit fix); `tasks.list?group_id=…` sibling query; Console-side `AwaitTask` orphan-detector cross-check; `tasks.list` row-shape enrichments (`Progress` / `Tags` / `LastActivityAt` / `IsBackground` / `HasPendingApproval`). Bulk-action toolbar invokes shipped Phase 54 verbs per row — no bulk endpoint (§13). | 73d (tasks.list shape), 75 |
...
| **73j** **(Shipped — D-118)** | Memory | `memory.list` + `memory.get` + `memory.health`; consumers for `memory.identity_rejected` (D-033) + `memory.recovery_dropped` (D-035) events. NO new scope minted — cross-tenant memory listing gates on the D-079 closed `auth.ScopeAdmin` set (audit B1). Page-memory.md §12 names the recovery event `memory.overflow_drop_oldest`; the shipped runtime wire string is `memory.recovery_dropped` — a docs(design) follow-up reconciles the §12 wording. | 72a, 75 |
...
**Console design-system foundation landed (2026-05-20).** The shared Console foundation — one SvelteKit route group `web/console/src/routes/(console)/` (no `/console/` URL prefix), one app shell, the shared `web/console/src/lib/components/ui/` inventory, the four-state `<PageState>` async contract, the unified `HarborClient` + `connection.ts`, and the reconciled `tokens.css` scale — is now settled in `docs/design/console/CONVENTIONS.md` (D-121). Every remaining not-yet-built Console page plan (73a, 73b, 73c, 73d, 73e, 73g, 73h, 73m, 73n) now carries a binding **"Console consistency"** section that cites `CONVENTIONS.md` and D-121; a page PR that diverges from a convention is rejected on sight. Where an older route literal in a page plan still reads `/console/<page>`, the "Console consistency" section is the governing instruction — pages route under `(console)/` with no `/console/` URL prefix.
