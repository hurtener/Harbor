...
below is **rejected on sight**. Detail routes use `(console)/sessions/[id]/` and the tenant facet is a `FilterBar` chip — never a forked filter strip.

The page MUST:

- **Route under `(console)/`.** The page lives at
  `web/console/src/routes/(console)/sessions/` and is served at `/sessions` with
  **no `/console/` URL prefix** (the `(console)` route group is a
  ...
  `(console)/sessions/[id]/` and are served at `/sessions/<id>`. All inter-page
...
- **Use the shared `components/ui/` inventory.** It composes the cross-page
  ...
  page-specific components go in `components/sessions/`.
