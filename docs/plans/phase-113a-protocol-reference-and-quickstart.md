...
None at planning time. Three §4.3 deviations recorded at ship (D-209 calls 3–4 plus the checkpoint-audit amendment below):
...
- **Quickstart smoke asserts shape via inline `jq`, not `assert_json_path` (recorded at the Protocol-track §17.5 checkpoint).** The criterion originally read "asserting status + response shape (`assert_json_path`)". The shipped smoke sources the page's own curl blocks so variables (token, task id) flow between steps — the `common.sh` wrappers cannot run inside those sourced blocks, so steps 1/2/4 assert shape with inline `jq` (an error envelope lacks the asserted fields, so a non-200 still fails loudly) and step 5 branches on its captured HTTP status. Same assertion strength, different mechanism; the criterion wording above was amended to match.
...
- [ ] **Executed quickstart:** `scripts/smoke/phase-113a.sh` extracts the quickstart's tagged curl steps and runs them in order against the preflight dev server — bootstrap → start → SSE tail observes ≥1 event for the started run → `tasks.get` → one steering call — each step asserting response shape via inline `jq` checks inside the sourced page blocks (an error envelope lacks the asserted fields, so a non-200 step fails loudly; the steering step additionally branches on its captured HTTP status). The `common.sh` wrappers (`assert_json_path`) cannot run inside the page's own sourced blocks because variables flow between steps — see the §4.3 deviation note below. FAIL (not SKIP) when a step's page block and the wire disagree on a build where the surface exists.
