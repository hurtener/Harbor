...
- **A create-if-absent precondition.** ~~"There is no active revision and I expect none" has no expressible value in a hash-typed token (empty means unconditional), and inventing a second field (`expect_absent`) to arbitrate a first-write race would be a second mechanism for a much narrower window. A token supplied when no active revision exists is a conflict; stated, tested, and left there.~~ **Superseded in-wave by D-370.** The non-goal was reversed once the window was reproduced: contributor B reads `set=false`, A writes, B writes with the only token it can express, and B silently reverts A with both told `200`. The fix is neither a second field nor a zero-state token from the read — it is a RESERVED SENTINEL inside the existing field, `expected_content_hash: "-"` (`agentcfg.ExpectNoActiveRevision`), which succeeds only while no active revision exists and is refused `revision_conflict` the moment one does, on both the content-write door and the pointer-move door. It cannot collide with a real token because a real one is exactly 64 lowercase hex characters, asserted by a `hashShaped` conformance row in both directions. Absent and 64-hex tokens are byte-for-byte unchanged.
...
- **The first-write race is arbitrated as of D-370** (this was authored as an accepted residual and did not survive the wave). The shape is NOT the `expect_absent` boolean this plan anticipated — that would have been a second field answering one question — but the reserved sentinel `expected_content_hash: "-"` inside the existing field. Three conformance rows carry it (`FirstWriteSentinel`, `FirstWriteSentinelRefusedOnceSet`, `FirstWriteSentinelPreventsLostUpdate`, run under both scope arms), the last of which reproduces the lost update and then shows it closed. The three arms live in ONE shared `checkExpectation` helper both doors call, so they can never disagree on what a token means.
- **The door count is SEVENTEEN as of v1.25, and the count is the risk.** It shipped at sixteen; phase 222's `agent_config.set_extra_system_blocks` made it seventeen, and this phase's exact-count guards caught it (three FAILs, "17 found, 16 wanted") — which is precisely what they are for. An eighteenth spine writer added without the field silently reopens the gap. Three guards: the exact-count smoke assertion, the `TestConditionalWrite_AllSeventeenDoorsAcceptTheToken` table, and a behavioural table that drives each door with a stale token — the only one of the three that catches a door which declares the field and then drops it on the floor. The count is asserted, not documented. `scripts/smoke/phase-221.sh`'s conformance-row guards separately moved 4 to 7 with D-370's first-write rows.
