...
- [ ] Issued tokens live in an in-memory TTL cache only. TTL = min(broker `expires_in`, `extra.cache_ttl_cap` default 5m, floored at an implementation-chosen minimum ≥30s). `TokenStore.Put` is NEVER invoked — asserted by test, not just by review. Refresh is re-exchange, collapsed under a single-flight gate keyed `(scope, subject, source)`.
...
- **Conformance:** join `internal/tools/auth/conformancetest` for the `Token` identity rules; interactive-flow suite legs assert `ErrNonInteractive` (typed) rather than skipping.
