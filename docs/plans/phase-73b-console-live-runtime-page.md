...
below is **rejected on sight**. The topology canvas is this page's primary view in the §5 depth bar (in place of a `DataTable`); it still mounts inside the shared app shell, routes its async state through the four-state `<PageState>` with a canvas-shaped Loading skeleton, and pairs with a `DetailRail`.

The page MUST:

- **Route under `(console)/`.** The page lives at
  `web/console/src/routes/(console)/live-runtime/` and is served at `/live-runtime` with
  **no `/console/` URL prefix** (the `(console)` route group is a
  ...
  `(console)/live-runtime/[id]/` and are served at `/live-runtime/<id>`. All inter-page
...
- **Use the shared `components/ui/` inventory.** It composes the cross-page
  ...
  page-specific components go in `components/live-runtime/`.
