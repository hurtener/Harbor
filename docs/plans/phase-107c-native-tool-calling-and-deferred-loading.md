...
**Scope honesty:** this is a large plan — ~30 ACs across the LLM client, tools subsystem, React planner, prompt builder, repair loop, and agent-yaml config. Realistic single-agent dispatch is 5–10 days of work. It ships the right long-term architecture in one cutover; Phase 107b's `streamAnswerFilter` becomes unnecessary and is NOT built. If the implementing agent runs into a blocking unknown, they should pause and ask rather than ship a partial migration.
...
- [ ] **AC-19** Decision mapping in React's response handler:
  ...
  - `len(resp.ToolCalls) > 1` → `CallParallel{Branches: [for each tc: CallTool{...}], Join: JoinAll, MaxParallel: rc.Hints.MaxParallel}`.
...
**Pause-and-ask checkpoints:** if any of the following surprises arise, the implementing agent should pause and ask before proceeding:
- A provider in the test surface rejects parallel tool-calls (the `ParallelToolCalls: true` knob's mapping fails).
- The mechanical fixture rewrite (step 9) finds tests asserting on parser-specific behavior that can't trivially translate to the structured form.
- The discovery cycle test (step 11) reveals a same-turn race where the LLM calls a tool it just discovered — see Risks/open questions.
- `make preflight` fails on phases 54/58/59 due to single-source checker missing the new wire types (the agent must register `ToolCallStructured` + `ToolDeclaration` in `internal/protocol/singlesource/singlesource.go` if they're declared in `internal/protocol/types/`; if they live in `internal/llm/types.go`, no protocol single-source registration is needed — but verify).
