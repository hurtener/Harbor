...
## 12. Phase 108l rebuild note (D-184)

Phase 108l rebuilt this page to the carded, viewport-locked page-polish standard (the Events-108h / Background-Jobs-108j / Tools-108k composition) and landed the fleet-control Protocol surface. What changed against §3–§7 above:

- **Fleet control is now LIVE.** The five verbs (Pause / Drain / Restart / Force-Stop / Deregister) are real admin-gated Protocol methods — `agents.{pause,drain,restart,force_stop,deregister}` at `POST /v1/agents/{verb}`, each wrapping the shipped in-process `registry.*` verb (D-066). This supersedes the D-132/F4 deferral, where the buttons rendered disabled-with-tooltip "no `registry.*` Protocol surface". They are control-scope gated: enabled with the `admin` claim, disabled-with-tooltip without it — never a fabricated success (CLAUDE.md §13).
- **Pause / Restart are advisory (honest status).** The V1 registry Health enum has no "paused" state, so `agents.pause` / `agents.restart` emit their `agent.paused` / `agent.restart_requested` event but leave the observable status `active`; only `drain`→drained, `force_stop`→force_stopped, and `deregister`→deregistered transition it. The control response re-reads and returns the ACTUAL status; the UI reflects it truthfully and treats the emitted event (seen live on the activity feed) as the observation of pause/restart. The §7 "Health badge" set (Healthy / Degraded / Paused / Drained / Force-Stopped) describes the badge labels, not a registry "paused" Health.
- **Recent activity is wired live.** The §5 `[shipped]` "Recent activity panel" now consumes a live `agent.*` `events.subscribe` stream filtered to the agent's payload `agent_id`, honest-empty when the stream is quiet (the runtime has no durable event read-back — it is a live-only view).
- **Layout = the §4 three-column main canvas** (tabbed detail / topology / activity+tools+memory), carded + viewport-locked, with the per-page `PageHeader` dropped (breadcrumb / ⌘K / footer are app-shell chrome, 108b). The §4 "Right rail: not used in detail" line holds. The two king files were refactored into `AgentsListPageState` / `AgentDetailPageState` controllers + a pure `derive.ts`.
- **Cross-page affordances (supersedes the §5 "Open in Playground → /console/live-runtime?agent=" row):** the detail header's **"Open in Playground"** now links to `/playground` (the session-scoped interactive Playground surface), not `/live-runtime` — the previous `?agent=` query param was silently dropped (neither route consumed it). The topology card's **"Open full topology →"** links to `/live-runtime`. V1 has no per-agent Playground/topology targeting (single-agent dev), so neither affordance carries an `agent` param it cannot honor (CLAUDE.md §13 — no dropped-param smell).

The URL prefix in the header line above predates the 108b chrome — the live route is `/agents` (list) + `/agents/[id]` (detail), with no `/console/` prefix (CONVENTIONS.md §1).
