...
- **Section: Storage drivers (per active runtime, read-only) — which `StateStore` / `ArtifactStore` / `MemoryStore` / `SkillStore` drivers are active.** `[shipped]` Derived from `types.VersionHandshake` extended OR a `runtime.drivers` snapshot method (NEW — Wave 13 to decide whether to extend `VersionHandshake` or add a separate snapshot).
...
| API tokens (per-user OAuth) section | `tool.auth_required` / `tool.auth_completed` events filtered to user; binding metadata from `tools.get` | Connect / Reconnect / Revoke per `ScopeUser` binding | `[shipped]` |
