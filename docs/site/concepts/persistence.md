...
- **Apply directly; verify through a transaction pool.** Every Postgres-backed store defaults `migration_mode` to `apply`, which takes a session advisory lock and needs a direct/session-capable endpoint. After a separate apply succeeds, steady-state replicas may use a transaction-pooled DSN with `migration_mode: verify`; that mode only reads the namespaced `harbor_schema_migrations` and `harbor_store_identity` ledgers plus required physical schema, performs no DDL/transaction/advisory lock, and fails loud if subsystem identity, checksums, or schema are missing or behind the binary's embedded migrations.

- **One database per runtime is the supported topology.** Point state, memory, artifacts, skills, `sessions.turns`, and `observability.rollups` at one logical database. Their migration histories remain independently namespaced in the fixed Harbor ledgers, so overlapping version numbers cannot satisfy another subsystem. Distinct per-subsystem DSNs remain a backward-compatible staged-rollout option, but they share one runtime-wide pool budget.
