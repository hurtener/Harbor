...
wire (`token_url` / `audience` / `scopes` / `remote{}`) instead of only a
...
server's own URL (never a wire field) and the wire `token_url` / `remote{}`
dials face the identical token-exchange SSRF backstop. The same opt-in is
