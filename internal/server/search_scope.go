// search_scope.go — the Protocol-side search ScopeChecker (the telemetry-assembly
// addendum follow-up).
//
...
// the direction rule (runtime packages import protocol TYPES
...
// it consults the auth-attached scope set on ctx via
// `protocolauth.HasScope`.
//
// The function is exported so the wiring (and tests that
// want the real shape) can inject it directly without re-implementing
// the closed-scope-set predicate.
//
// pins the closed two-scope set `{admin, console:fleet}` — BOTH
// are cross-tenant fan-in entitlements. The §17.5 checkpoint
// (a checkpoint fix) corrected this predicate to honour `console:fleet` too:
