	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				_, _ = r.GC(ctx, r.gcPolicy) //nolint:errcheck // best-effort sweep; transient GC error must not crash the sweeper (see comment above)
				cancel()
			}
		}
	}()
...
func (r *Registry) GC(ctx context.Context, policy GCPolicy) (int, error) {
	...
	for _, q := range pending {
		...
		_ = r.bus.Publish(ctx, events.Event{ //nolint:errcheck // best-effort event emit; publish failure must not fail GC
