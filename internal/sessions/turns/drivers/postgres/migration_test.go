...
// verify schema_migrations contains exactly one row at version 1 and
// both driver tables exist.
...
// must still contain exactly one row at version 1.
...
// tableExists reports whether a relation named table exists in the
// DSN's search path.
func tableExists(t *testing.T, dsn, table string) bool {
	t.Helper()
	db, err := sql.Open("pgx", dsn)
	if err != nil {
		t.Fatalf("tableExists sql.Open: %v", err)
	}
	defer func() { _ = db.Close() }()
	ctx, cancel := context.WithTimeout(context.Background(), defaultTestTimeout)
	defer cancel()
	...
	err = db.QueryRowContext(ctx, `
        ...
        WHERE table_name = $1`, table).Scan(&one)
