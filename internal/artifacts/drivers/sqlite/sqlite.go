...
// selectRef returns the ref at (triple, id) if present. The primary key
// admits at most one row per triple + namespace + id, so the `LIMIT 1`
// is a guard rather than a choice between candidates.
