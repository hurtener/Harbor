...
-- The same rule is applied by the filesystem driver's index rebuild and
-- by the object-store driver's key resolution, so all three agree. The
-- bytes are identical across the collapsed rows by content-addressing;
-- only the provenance stamp is at stake.
--
-- SQLite cannot ALTER a PRIMARY KEY, so the table is rebuilt. The runner
-- executes this body inside one transaction.
...
CREATE TABLE artifacts_blobs_triple_keyed (
...
INSERT INTO artifacts_blobs_triple_keyed
...
DROP TABLE artifacts_blobs;

ALTER TABLE artifacts_blobs_triple_keyed RENAME TO artifacts_blobs;
