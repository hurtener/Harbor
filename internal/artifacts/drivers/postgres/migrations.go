...
// deployment.
//
// pg_advisory_lock takes a signed int64; FNV-64a returns uint64. The
// bit-pattern reinterpretation is intentional and safe — Postgres
// hashes the bits, not their numeric value.
var advisoryLockKey = fnv64aSigned("harbor-artifacts-migrations")
...
// error (a bad filename in the embed set is a build-time bug).
func loadMigrations() ([]migration, error) {
	entries, err := fs.ReadDir(migrationsFS, "migrations")
	if err != nil {
		return nil, fmt.Errorf("artifacts/postgres: read embedded migrations: %w", err)
	}
	...
	for _, e := range entries {
		...
		if !ok || len(vstr) != 4 {
			return nil, fmt.Errorf("artifacts/postgres: malformed migration filename %q (want NNNN_*.sql)", e.Name())
		}
		v, err := strconv.Atoi(vstr)
		if err != nil {
			return nil, fmt.Errorf("artifacts/postgres: malformed migration version in %q: %w", e.Name(), err)
		}
		body, err := fs.ReadFile(migrationsFS, "migrations/"+e.Name())
		if err != nil {
			return nil, fmt.Errorf("artifacts/postgres: read migration %q: %w", e.Name(), err)
		}
...
//
// The lock is held until the end of the function; pg_advisory_unlock
// is called explicitly so a panic in the migration body still drops
// the lock at session-close time.
func applyMigrations(ctx context.Context, db *sql.DB) error {
	...
	if len(migs) == 0 {
		return errors.New("artifacts/postgres: no migrations found in embedded migrations/ — package mis-built")
	}
	...
	if err != nil {
		return fmt.Errorf("artifacts/postgres: acquire migration conn: %w", err)
	}
	defer func() { _ = conn.Close() }()

	if _, lockErr := conn.ExecContext(ctx, "SELECT pg_advisory_lock($1)", advisoryLockKey); lockErr != nil {
		return fmt.Errorf("artifacts/postgres: pg_advisory_lock: %w", lockErr)
	}
	defer func() {
	...
	`); bootErr != nil {
		return fmt.Errorf("artifacts/postgres: create schema_migrations bootstrap: %w", bootErr)
	}
	...
	for _, m := range migs {
		...
		if err := applyOne(ctx, conn, m); err != nil {
			return fmt.Errorf("artifacts/postgres: apply migration %s: %w", m.name, err)
		}
	}
	return nil
}
...
func loadAppliedVersions(ctx context.Context, conn *sql.Conn) (map[int]struct{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("artifacts/postgres: select schema_migrations: %w", err)
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&v); err != nil {
			return nil, fmt.Errorf("artifacts/postgres: scan schema_migrations: %w", err)
		}
		out[v] = struct{}{}
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("artifacts/postgres: iterate schema_migrations: %w", err)
	}
	return out, nil
}
