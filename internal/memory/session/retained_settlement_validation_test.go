		...
		t.Run(driver, func(t *testing.T) {
			for _, extra := range []string{
				...
				t.Run(extra, func(t *testing.T) {
					...
					r, err := sessionmemory.BeginRetainedRun(t.Context(), store, settlementRedactor{Redactor: redactor, extra: extra}, base.Quadruple, 2, time.Hour, nil)
					if err != nil {
						t.Fatal(err)
					}
					...
					if err = r.AfterDispatch(t.Context(), base, step); !errors.Is(err, sessionmemory.ErrRetainedContextUnavailable) {
		...
		t.Run(driver, func(t *testing.T) {
			for _, scenario := range []string{"different expiry", "oversized intent"} {
				t.Run(scenario, func(t *testing.T) {
					...
					r, err := sessionmemory.BeginRetainedRun(t.Context(), wrapper, redactor, base.Quadruple, 2, time.Hour, nil)
					if err != nil {
						t.Fatal(err)
					}
					if err = r.Apply(&base); err != nil {
