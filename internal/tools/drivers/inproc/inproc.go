...
func RegisterFunc[I any, O any](
	...
	descriptor := tools.ToolDescriptor{
		...
		// Tool-lifecycle events (tool.invoked / .completed / .failed) are
		// NOT emitted here per-driver — the catalog wraps every
		// registered descriptor's Invoke in the ONE universal
		// lifecycle-emitting shell at Register time, so every transport
		// (and every dispatch call site) gets them by construction and
