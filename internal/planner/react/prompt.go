...
- skill_search(query, limit): search skill playbooks by topic. Results are capability-filtered to what this run can actually use.
- skill_get(names[], max_tokens): fetch one or more skills' full content, budget-fit to max_tokens.
- skill_list(scope, task_type, tags[], limit, offset): enumerate the skill catalog, summary-only.
