...
func makeEnv() messages.Envelope {
...
func TestNodePolicy_ZeroValue_BareWorker(t *testing.T) {
	...
	out, err := runWithReliability(context.Background(), makeEnv(), fn, NodePolicy{}, nil, "n", nil)
	if err != nil {
...
func TestNodePolicy_ValidateBoth_RejectsMalformedIn(t *testing.T) {
	...
	_, err := runWithReliability(context.Background(), makeEnv(), fn, policy, nil, "n", nil)
	var re *RunError
	if !errors.As(err, &re) {
...
func TestNodePolicy_ValidateNone_SkipsValidator(t *testing.T) {
	...
	if _, err := runWithReliability(context.Background(), makeEnv(), fn, policy, nil, "n", nil); err != nil {
		t.Fatalf("err=%v, want nil", err)
	}
...
func TestNodePolicy_Timeout_ProducesRunError(t *testing.T) {
	...
	start := time.Now()
	_, err := runWithReliability(context.Background(), makeEnv(), fn, policy, nil, "slow", nil)
...
func TestNodePolicy_MaxRetries_StopsAfterN(t *testing.T) {
	...
	_, err := runWithReliability(context.Background(), makeEnv(), fn, policy, nil, "fail", nil)
	if calls.Load() != 4 {
...
func TestNodePolicy_CtxCancelled_AbortsRetries(t *testing.T) {
	...
	go func() {
		_, err := runWithReliability(ctx, makeEnv(), fn, policy, nil, "n", nil)
...
func TestNodePolicy_CtxCancelled_BeforeFirstAttempt(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	_, err := runWithReliability(ctx, makeEnv(), fn, NodePolicy{}, nil, "n", nil)
	var re *RunError
	if !errors.As(err, &re) {
		t.Fatalf("err=%v, want *RunError", err)
	}
...
func TestNodePolicy_RetrySuccess(t *testing.T) {
	...
	policy := NodePolicy{MaxRetries: 3, BackoffBase: 0}
	out, err := runWithReliability(context.Background(), makeEnv(), fn, policy, nil, "n", nil)
	if err != nil {
		t.Fatalf("err=%v, want nil", err)
	}
	if out.Payload != "p" {
...
func TestNodePolicy_PanicRecovery(t *testing.T) {
	...
	_, err := runWithReliability(context.Background(), makeEnv(), fn, NodePolicy{}, nil, "n", nil)
	var re *RunError
	if !errors.As(err, &re) {
		t.Fatalf("err=%v, want *RunError", err)
	}
