...
// Decision shape, and the runtime executor (a
...
// intact (the master-plan flags as a critical-path phase for
// exactly this reason).
//
// # Durability
//
...
// existing state.StateStore, handed to the Coordinator as
...
// configured."
//
// deliberately does NOT mint a second persistence-driver
...
// two-parallel-implementations smell.
//
// # Fail loudly
//
...
//
// Harbor ships the primitive. The first end-to-end
// RequestPause-driven-through-the-Coordinator consumer is the run-loop wiring
// (steering wiring) — same wave,. also
// consumes this package's surface (the pause-record serialise
// contract). See the phase plan's "§13 primitive-with-consumer
// obligation" section.
package pauseresume
...
// (RFC §6.3): the planner package is the truth source for the
...
// (Harbor uses a ULID source). A Token is unique per Request call.
type Token string
...
type Status struct {
	...
	// (approve / reject / resume / timeout); the zero value
	// while State == StatusPaused. Lets an in-process observer (the
	// steering RunLoop's timeout detection —)
...
type Coordinator interface {
	...
	// the caller's identity scope (the `pause.list`
