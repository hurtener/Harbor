...
// decide whether a tenant-owned MCP source is currently selected. The concrete
...
// a projection over the existing owning config revision, not a second
...
// ErrSourceAuthorityUnavailable means a tenant-owned source was observed but no
// durable agent/user config reader was wired. User-owned sources fail closed in
// this posture; owner filtering alone is only a cache privacy check and is not
// an authorization decision.
var ErrSourceAuthorityUnavailable = errors.New("mcpconsole: owned-source authority unavailable")
...
// the current owning agent/user revision remains the sole selection authority.
...
// agent/user config reader. A nil config is intentionally retained: ownerless
// boot sources remain compatible, while tenant-owned sources fail closed with
...
// caller after the registry's identity check. A tenant-owned source is visible
// only when its exact logical pair is present in the caller's current
// owning config revision. The session id is deliberately absent from this
// durable pair match.
func (a *SourceAuthorizer) Visible(ctx context.Context, source tools.ToolSourceID, effectiveAgent string) (bool, error) {
	...
	if err != nil {
		// SourceAccess deliberately collapses a foreign tenant-owned source
...
// must not synthesize either value. It shares the exact same owning-scope
...
func (a *SourceAuthorizer) VisibleRegistration(ctx context.Context, _ tools.ToolSourceID, owner auth.Owner, logical, effectiveAgent string) (bool, error) {
	...
	if !owner.AllowsPrincipal(id.TenantID, id.UserID) {
		return false, nil
	}
	...
	scope := agentcfg.ConfigScopeUser
	if owner.Scope() == auth.ScopeTenantAgent {
		scope = agentcfg.ConfigScopeAgent
	}
	active, has, err := a.config.Active(ctx, identity.Quadruple{Identity: id}, owner.Agent, scope)
	if err != nil {
		return false, fmt.Errorf("read active source config: %w", err)
	}
	if !has {
		return false, nil
	}
	if scope == agentcfg.ConfigScopeAgent {
		for _, descriptor := range active.Payload.ConnectionDescriptors() {
			if descriptor.Name == logical {
				return true, nil
			}
		}
	}
