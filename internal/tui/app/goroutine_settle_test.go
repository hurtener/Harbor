...
// a program mount/unmount leaves its reader/renderer goroutines unwinding for a
// scheduler tick or two after Run returns, so a single instant sampled right
// after can read transiently over tolerance. Each attempt runs runtime.GC() —
...
// only if the count never drains, so a genuine leak still fails the test.
