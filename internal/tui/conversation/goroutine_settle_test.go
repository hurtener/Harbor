...
// over tolerance while their goroutines are still unwinding. Each attempt runs
// runtime.GC() — reaping finished-but-unscheduled goroutines and parking the
// poller so the exiting goroutines get scheduler time under load — then
// re-samples. It fails only if the count never drains, so a genuine leak still
// fails the test.
