...
const (
	...
	// agentCfgUser is the reserved synthetic user slot every agentcfg
	// record persists under. The double-underscore prefix is reserved for
...
func (r *registry) SetRevision(ctx context.Context, id identity.Quadruple, agentID string, payload agentcfg.ConfigPayload) (agentcfg.Revision, error) {
	...
	active, hasActive, err := r.loadActiveRevision(ctx, q)
	if err != nil {
		return agentcfg.Revision{}, err
	}
	parentID := ""
	...
	if err := r.saveRevision(ctx, q, rec); err != nil {
		return agentcfg.Revision{}, err
	}
	if err := r.saveActive(ctx, q, revID, now); err != nil {
		return agentcfg.Revision{}, err
	}
	rev := rec.toRevision()
...
func (r *registry) Active(ctx context.Context, id identity.Quadruple, agentID string) (agentcfg.Revision, bool, error) {
	if err := r.validate(id, agentID); err != nil {
	...
	return r.loadActiveRevision(ctx, syntheticQuad(id.TenantID, agentID))
}

func (r *registry) Get(ctx context.Context, id identity.Quadruple, agentID, revisionID string) (agentcfg.Revision, error) {
	if err := r.validate(id, agentID); err != nil {
		return agentcfg.Revision{}, err
	}
	if revisionID == "" {
	...
	rec, err := r.loadRevision(ctx, syntheticQuad(id.TenantID, agentID), revisionID)
	if err != nil {
		return agentcfg.Revision{}, err
	}
...
func (r *registry) ListRevisions(ctx context.Context, id identity.Quadruple, agentID string, limit int) ([]agentcfg.Revision, error) {
	if err := r.validate(id, agentID); err != nil {
		return nil, err
	}
	if err := ctx.Err(); err != nil {
	...
	q := syntheticQuad(id.TenantID, agentID)
	recs, err := r.state.ListKind(ctx, state.ListScope{MaintenanceScoped: true}, kindRevisionPfx)
	if err != nil {
...
func (r *registry) Rollback(ctx context.Context, id identity.Quadruple, agentID, revisionID string) (agentcfg.Revision, error) {
	if err := r.validate(id, agentID); err != nil {
		return agentcfg.Revision{}, err
	}
	if revisionID == "" {
	...
	target, err := r.loadRevision(ctx, q, revisionID)
	if err != nil {
		return agentcfg.Revision{}, err
	}
	fromID := ""
	if active, hasActive, aerr := r.loadActiveRevision(ctx, q); aerr == nil && hasActive {
		fromID = active.RevisionID
	}
	now := r.clock().UTC()
	if err := r.saveActive(ctx, q, revisionID, now); err != nil {
		return agentcfg.Revision{}, err
	}
...
func (r *registry) Diff(ctx context.Context, id identity.Quadruple, agentID, fromRev, toRev string) (agentcfg.Diff, error) {
	if err := r.validate(id, agentID); err != nil {
	...
	q := syntheticQuad(id.TenantID, agentID)
	from, err := r.loadRevision(ctx, q, fromRev)
	if err != nil {
		return agentcfg.Diff{}, err
	}
	to, err := r.loadRevision(ctx, q, toRev)
	if err != nil {
		return agentcfg.Diff{}, err
	}
...
func (r *registry) loadActiveRevision(ctx context.Context, q identity.Quadruple) (agentcfg.Revision, bool, error) {
	...
	rr, err := r.loadRevision(ctx, q, ar.RevisionID)
	if err != nil {
...
func (r *registry) loadRevision(ctx context.Context, q identity.Quadruple, revisionID string) (revisionRecord, error) {
	rec, err := r.state.Load(ctx, q, kindRevisionPfx+revisionID)
	if err != nil {
		if errors.Is(err, state.ErrNotFound) {
...
func (r *registry) saveRevision(ctx context.Context, q identity.Quadruple, rec revisionRecord) error {
	...
	if err := r.state.Save(ctx, state.StateRecord{
		...
		Identity:  q,
		Kind:      kindRevisionPfx + rec.RevisionID,
		Bytes:     buf,
...
func (r *registry) saveActive(ctx context.Context, q identity.Quadruple, revisionID string, now time.Time) error {
