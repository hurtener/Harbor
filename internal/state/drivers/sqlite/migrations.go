...
// runtime-discovered SQL files (RFC §10 — keep the toolchain footprint
// minimal at V1).
//
...
//
// The runner additionally guards each file with a precheck against
// `schema_migrations` so a partially-applied DB (where a previous
// run applied DDL but the trailing INSERT failed) is still
// recoverable. Each migration runs inside a single transaction so
// either all of its statements apply or none do.
func migrate(ctx context.Context, db *sql.DB) error {
	...
	for _, f := range files {
		...
		if err := applyMigration(ctx, db, f); err != nil {
			return fmt.Errorf("state/sqlite: apply migration %s: %w", f.name, err)
		}
	}
	return nil
}
...
func listMigrations() ([]migrationFile, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("state/sqlite: read embedded migrations: %w", err)
	}
	...
	for _, e := range entries {
		...
		if m == nil {
			return nil, fmt.Errorf("state/sqlite: migration %q does not match NNNN_<slug>.sql", e.Name())
		}
		v, err := strconv.Atoi(m[1])
		if err != nil {
			return nil, fmt.Errorf("state/sqlite: migration %q has unparseable version: %w", e.Name(), err)
		}
...
func ensureMigrationsTable(ctx context.Context, db *sql.DB) error {
	...
	if err != nil {
		return fmt.Errorf("state/sqlite: bootstrap schema_migrations: %w", err)
	}
	return nil
}
...
func loadAppliedVersions(ctx context.Context, db *sql.DB) (map[int]struct{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("state/sqlite: read schema_migrations: %w", err)
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&v); err != nil {
			return nil, fmt.Errorf("state/sqlite: scan schema_migrations.version: %w", err)
		}
		out[v] = struct{}{}
	}
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("state/sqlite: iterate schema_migrations: %w", err)
	}
	return out, nil
}
