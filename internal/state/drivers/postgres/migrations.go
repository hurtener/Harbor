...
// subsystem so the FNV-derived key never collides with another subsystem.
func applyMigrations(ctx context.Context, db *sql.DB) error {
	return sqlmigrate.RunPostgres(ctx, db, migrationsFS, "postgres", "harbor-state-migrations")
}
