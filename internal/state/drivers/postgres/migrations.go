...
var advisoryLockKey = fnv64aSigned("harbor-state-migrations")
...
func loadMigrations() ([]migration, error) {
	...
	for _, e := range entries {
		...
		if !ok || len(vstr) != 4 {
			return nil, fmt.Errorf("postgres: malformed migration filename %q (want NNNN_*.sql)", e.Name())
		}
		v, err := strconv.Atoi(vstr)
		if err != nil {
			return nil, fmt.Errorf("postgres: malformed migration version in %q: %w", e.Name(), err)
		}
		body, err := fs.ReadFile(migrationsFS, "migrations/"+e.Name())
		if err != nil {
			return nil, fmt.Errorf("postgres: read migration %q: %w", e.Name(), err)
		}
...
func applyMigrations(ctx context.Context, db *sql.DB) error {
	...
	if len(migs) == 0 {
		return errors.New("postgres: no migrations found in embedded migrations/ — package mis-built")
	}
	...
	if _, lockErr := conn.ExecContext(ctx, "SELECT pg_advisory_lock($1)", advisoryLockKey); lockErr != nil {
		return fmt.Errorf("postgres: pg_advisory_lock: %w", lockErr)
	}
	defer func() {
	...
	`); bootErr != nil {
		return fmt.Errorf("postgres: create schema_migrations bootstrap: %w", bootErr)
	}
...
func loadAppliedVersions(ctx context.Context, conn *sql.Conn) (map[int]struct{}, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("postgres: select schema_migrations: %w", err)
	}
