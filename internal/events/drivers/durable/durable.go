...
func (b *bus) publishDirect(ctx context.Context, ev events.Event) error {
	...
	generation, epochID, err := b.generationForAdmission(ctx, ev.Identity.Identity)
	if err != nil {
		return fmt.Errorf("durable: capture session erasure epoch: %w", err)
	}
	...
	if err := b.sequenceAndStore(ctx, &ev, generation, epochID); err != nil {
		if errors.Is(err, errEventFenced) {
...
func (b *bus) sequenceAndStore(ctx context.Context, ev *events.Event, generation uint64, expectedEventID string) error {
	b.publishMu.Lock()
	defer b.publishMu.Unlock()
	if !b.generationMatchesLocked(ev.Identity, generation) {
		return errEventFenced
	}
	if !b.bestEffort {
		changed, err := b.erasureEpochChanged(ctx, ev.Identity.Identity, generation, expectedEventID)
		if err != nil {
			return err
		}
		if changed {
	...
	seq, err := b.persistAtomic(ctx, ev, generation, expectedEventID)
	if err != nil {
		if errors.Is(err, errEventFenced) {
			return err
		}
...
func (b *bus) persistAtomic(ctx context.Context, ev *events.Event, generation uint64, expectedEventID string) (uint64, error) {
	for attempt := 1; attempt <= publishCASMaxAttempts; attempt++ {
		if err := ctx.Err(); err != nil {
			return 0, err
		}
		...
		ev.Sequence = seq
		if err := b.persistAtomicAttempt(ctx, *ev, authorityExpectation, expectedEventID); err != nil {
			if errors.Is(err, state.ErrConditionFailed) {
				changed, checkErr := b.erasureEpochChanged(ctx, ev.Identity.Identity, generation, expectedEventID)
				if checkErr != nil {
					return 0, checkErr
				}
				if changed {
					return 0, errEventFenced
				}
				continue
			}
			return 0, err
...
func (b *bus) persistAtomicAttempt(ctx context.Context, ev events.Event, authorityExpectation state.SlotExpectation, expectedEventID string) error {
	entryBytes, err := encodeEvent(ev)
	if err != nil {
		return err
	}
	...
	epochExpectation := state.InternalSlotExpectation(sequenceAuthorityIdentity, durableEpochKind(ev.Identity.Identity), state.EventID(expectedEventID))
	...
	return b.store.SaveBatchIf(ctx,
		[]state.SlotExpectation{authorityExpectation, entryExpectation, headExpectation, fenceExpectation, epochExpectation},
