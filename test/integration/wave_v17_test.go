...
func TestE2E_WaveV17_CombinedSurface(t *testing.T) {
	...
	t.Run("BothCapabilitiesCoexist", func(t *testing.T) {
		...
		// advertised capability is canonical, and the canonical set is 16
		// (Phase 177 / D-313 added tool_annotations; D-374 added
		// caller_memory; HA-68 added skill_publications; the provider
		// catalog, route and atomic run settings are additive as well).
		for _, c := range info.Capabilities {
			if !prototypes.IsValidCapability(c) {
				t.Errorf("advertised capability %q is not in the canonical universe", c)
			}
		}
		for _, capability := range []prototypes.Capability{"tenant_scoped_broker_credentials_v1", "tools_configuration_view_v1", "run_llm_settings_v1"} {
			if !prototypes.IsValidCapability(capability) {
				t.Fatalf("missing canonical capability %s", capability)
			}
		}
		if n := len(prototypes.Capabilities()); n != 16 {
			t.Errorf("canonical capability universe has %d entries, want 16", n)
		}
	})
