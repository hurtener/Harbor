...
// TestDevStack_SkipTransports_BootIndexStillLoaded is the P1 regression
// for the index-loading guard: loading/validation/pre-read must be
// independent of transport construction. A `SkipTransports: true` stack
// (no mux, no listener) that declares boot packs MUST still open the
// eager index and hand the SAME frozen reader to the run-loop driver
// before it starts — a deliberately component-skipped stack may fail loud
// if a mandatory catalog / registry collaborator is absent, but it may
// never silently start a driver with no baseline just because the
// transport leg was skipped.
func TestDevStack_SkipTransports_BootIndexStillLoaded(t *testing.T) {
	root := t.TempDir()
	dir := filepath.Join(root, "foundation")
	if err := os.MkdirAll(dir, 0o755); err != nil {
		t.Fatal(err)
...
`
	...
	stack, err := TryAssemble(cfg, AssembleOpts{SkipTransports: true})
	if err != nil {
		if stack != nil {
			stack.Close()
		}
		t.Fatalf("TryAssemble (SkipTransports, boot baseline declared): %v", err)
	}
	defer stack.Close()

	if stack.Handler != nil || stack.Mux != nil {
		t.Fatal("SkipTransports stack unexpectedly mounted transports")
	}
	if stack.RunLoopDriver == nil {
		t.Fatal("devstack assembled without a run-loop driver")
	}
	reader := stack.RunLoopDriver.BootPackReader()
	if reader == nil {
		t.Fatal("SkipTransports run-loop driver has no boot-pack reader — the eager index was not loaded without transports")
	}
	entries, ok := reader.Lookup(DefaultDevTenant, devAgentConfigID)
	if !ok || len(entries) != 1 {
		t.Fatalf("SkipTransports driver boot reader Lookup(%q, %q) = %d entries ok=%v, want the ONE frozen boot entry",
			DefaultDevTenant, devAgentConfigID, len(entries), ok)
	}
	if entries[0].Skill.Name != "workbench-foundation" {
		t.Fatalf("SkipTransports driver boot reader entry name = %q, want workbench-foundation", entries[0].Skill.Name)
	}
}

// TestTryAssemble_NoBootPacks_DriverReaderNil pins the P0 no-baseline
// shape at the driver seam: with NO boot declarations the run-loop
// driver's BootPackReader must be an ACTUAL nil (the compatible
// pre-baseline shape) — never a non-nil interface holding a typed-nil
// `*bootpacks.Index`, whose Lookup would panic at the first run start.
func TestTryAssemble_NoBootPacks_DriverReaderNil(t *testing.T) {
	...
	if reader := stack.RunLoopDriver.BootPackReader(); reader != nil {
		t.Fatalf("no-boot driver BootPackReader = non-nil interface (%T), want an actual nil — the typed-nil P0 shape", reader)
	}
}
