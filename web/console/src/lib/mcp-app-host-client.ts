...
export function makeMCPAppHostClient(client: ProtocolClient): MCPAppHostClient {
  ...
  return {
	...
	async callTool(serverID, tool, args, agentID, binding): Promise<MCPAppToolResult> {
      let res: MCPAppCallToolResponse;
      try {
		res = await client.mcp.apps.callTool<MCPAppCallToolResponse>(serverID, tool, args, agentID, binding);
      } catch (err) {
